                    ...
                    for s in lista_sinais:
                        ...
                        if adicionar_historico(item):
                            try:
                                ...
                                # [CORREÇÃO 3] Envia TODOS os sinais (sem veto)
                                msg += f"\n\n🤖 <b>CLASSIFICAÇÃO IA: {opiniao_db}</b>"
                                
                                if opiniao_db == "Aprovado" or "DIAMANTE" in opiniao_db:
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"✅ Sinal Enviado: {s['tag']}")
                                elif opiniao_db == "Arriscado":
                                    msg += "\n⚠️ <i>Obs: Risco moderado detectado.</i>"
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"⚠️ Sinal Arriscado Enviado: {s['tag']}")
                                else:
                                    # Envia qualquer outra classificação também
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"📤 Sinal Enviado: {s['tag']}")
