...
# ==============================================================================
                    ...
                    for s in lista_sinais:
                        ...
                        if IA_ATIVADA:
                            try:
                                ...
                                if "diamante" in opiniao_txt.lower(): 
                                    opiniao_db = "Aprovado"
                                elif "aprovado" in opiniao_txt.lower(): 
                                    opiniao_db = "Aprovado"
                                elif "alto risco" in opiniao_txt.lower(): 
                        ...
                        if adicionar_historico(item):
                            try:
                                ...
                                # [CORRIGIDO v7.2] SEMPRE envia - apostador decide
                                if opiniao_db == "Aprovado":
                                    ...
                                    # Arriscado, Offline ou qualquer outro - ENVIA
                                    msg += "\n👀 <i>Obs: Risco moderado detectado pela IA.</i>"
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"⚠️ Sinal Enviado: {s['tag']}")
        ...
        st.error(f"⚠️ Erro no ciclo principal: {e_main}")
        st.info("🔄 O robô vai tentar novamente em 30 segundos...")
        st.caption(f"Detalhes: {type(e_main).__name__}: {e_main}")
        import traceback
        st.code(traceback.format_exc()[-500:], language="text")
        time.sleep(30)
        st.rerun()
else:
    with placeholder_root.container():
