...
def gerar_insights_matinais_ia(api_key):
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════════════
        try:
            texto_ia = validar_snipers_pos_gemini(texto_ia, mapa_jogos, api_key)
        except Exception as e:
...
def verificar_alerta_matinal(token, chat_ids, api_key):
    ...
    if 6 <= agora.hour < 12:  # [FIX] Matinal estendido até 12h
        ...
        if not st.session_state['matinal_enviado']:
            ...
            if eh_erro:
                print(f"[SNIPER MATINAL] ❌ Erro temporário, vai tentar de novo no próximo ciclo")
                # NÃO marca matinal_enviado = True → tenta de novo
            elif insights and "Sem jogos" not in str(insights) and "IA Offline" not in str(insights) and "dados suficientes" not in str(insights):
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
