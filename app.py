...
with st.sidebar:
    ...
    with st.expander("💰 Gestão de Banca", expanded=False):
        ...
        st.session_state['ia_profunda_ativada'] = st.checkbox("🧠 IA Tier 2 (H2H/Momentum)", value=st.session_state.get('ia_profunda_ativada', False), help="Só ativa em Ligas Seguras 🛡️. ~5 calls extras/sinal")
        if st.session_state.get('ia_profunda_ativada'):
            t2_used = st.session_state.get('tier2_calls_hoje', 0)
            st.caption(f"🧠 Tier 2: {t2_used} calls hoje (só ligas 🛡️)")
