...
with st.sidebar:
    ...
    with st.expander("💰 Gestão de Banca", expanded=False):
        ...
        if st.session_state.get('ia_profunda_ativada'):
            ...
            t2_max = st.session_state.get('tier2_max_dia', 500)
            st.progress(min(t2_used / max(t2_max, 1), 1.0))
            st.caption(f"🧠 Tier 2: {t2_used}/{t2_max} calls hoje (só 🛡️)")
                ...
                if lista_sinais:
                    ...
                    for s in lista_sinais:
                        ...
                        try:
                            ...
                            tier2_budget_ok = st.session_state.get('tier2_calls_hoje', 0) < st.session_state.get('tier2_max_dia', 500)
