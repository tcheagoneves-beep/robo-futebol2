...
# --- FUNÇÃO UNIVERSAL DE CHAVES (SEGURANÇA CONTRA DUPLICIDADE) ---
...
# --- FUNÇÕES DE LISTAS (BLACKLIST/SAFE/VIP) MANTIDAS IGUAIS ---
    ...
    if 'historico_full' not in st.session_state or force:
        ...
        if not df.empty and 'Data' in df.columns:
            ...
            for item in st.session_state['historico_sinais']:
                # Registra chaves históricas para evitar duplicação no reload
                fid_strat = gerar_chave_universal(item['FID'], item['Estrategia'], "SINAL")
