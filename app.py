...
def bf_login():
    ...
    try:
        ...
        if not all([username, password, app_key]):
            st.session_state['bf_erro'] = "Credenciais ausentes (USERNAME/PASSWORD/APP_KEY)"
            print("[BETFAIR] ❌ Credenciais ausentes")
            return False
        st.session_state['bf_erro'] = ""
        print(f"[BETFAIR] 🔄 Tentando login para: {username[:5]}***")

        client = None  # Será definido por cert ou REST
        ...
        if cert_crt and cert_key_pem and "BEGIN" in str(cert_crt):
            try:
                ...
                # Corrige PEM (header colado no body)
                crt_fixed = _fix_pem(cert_crt)
                key_fixed = _fix_pem(cert_key_pem)
                with open(crt_path, 'w') as f: f.write(crt_fixed)
                with open(key_path, 'w') as f: f.write(key_fixed)
                print(f"[BETFAIR] 🔐 Certs salvos ({len(crt_fixed)} / {len(key_fixed)} bytes)")

                # Login cert via REST (mais confiável que betfairlightweight.login)
                resp = requests.post(
                ...
                data = resp.json()
                print(f"[BETFAIR] 🔐 Cert login: status={data.get('loginStatus')}")
                if data.get("loginStatus") == "SUCCESS":
                    ...
                    print("[BETFAIR] ✅ Login via certificado OK!")
                else:
                    st.session_state['bf_erro'] = f"Cert: {data.get('loginStatus', 'FALHOU')}"
                    print(f"[BETFAIR] ⚠️ Cert login falhou: {data.get('loginStatus')}")
            except Exception as e_cert:
                st.session_state['bf_erro'] = f"Cert erro: {str(e_cert)[:100]}"
                print(f"[BETFAIR] ⚠️ Cert login erro: {e_cert}")
                client = None

        # ═══ TENTATIVA 2: Login interativo via REST (sem cert) ═══
        if client is None:
            print("[BETFAIR] 🔄 Tentando login interativo (REST)...")
            session_token = None
            endpoints = [
            ...
            for url in endpoints:
                try:
                    ...
                    error = data.get("error", data.get("loginStatus", "none"))
                    print(f"[BETFAIR] 📡 {url.split('/')[2]}: status={status}, error={error}")
                    if status == "SUCCESS":
                        session_token = data.get("token", data.get("sessionToken"))
                        break
                except Exception as e_url:
                    print(f"[BETFAIR] ⚠️ {url}: {e_url}")
            if not session_token:
                st.session_state['bf_erro'] = f"Login falhou em todos endpoints. Verifique credenciais."
                print("[BETFAIR] ❌ Login falhou em TODOS os endpoints")
                return False
            client = betfairlightweight.APIClient(
                username=username, password=password, app_key=app_key,
                lightweight=True
        ...
        try:
            ...
            print(f"[BETFAIR] ⚠️ Saldo indisponível: {e}")
            st.session_state['bf_saldo'] = 0.0
        print(f"[BETFAIR] ✅ Conectado! Token: {str(st.session_state['bf_session_token'])[:12]}...")
        return True
    except Exception as e:
                ...
                if st.button("🔗 Conectar Betfair"):
                    with st.spinner("Conectando à Betfair..."):
                        ...
                        if ok:
                            ...
                            st.error(f"❌ {erro}")
                # Mostra último erro persistente
