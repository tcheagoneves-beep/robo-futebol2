...
def bf_login():
    """Login na Betfair via API (usa secrets OU inputs manuais do UI)."""
    global BF_CONECTADO
    if not BF_AVAILABLE:
        print("[BETFAIR] ❌ betfairlightweight não instalado")
        return False
    try:
        # Tenta secrets primeiro, depois inputs manuais
        username = st.secrets.get("BF_USERNAME", "") if "BF_USERNAME" in st.secrets else st.session_state.get('bf_username_input', '')
        password = st.secrets.get("BF_PASSWORD", "") if "BF_PASSWORD" in st.secrets else st.session_state.get('bf_password_input', '')
        app_key = st.secrets.get("BF_APP_KEY", "") if "BF_APP_KEY" in st.secrets else st.session_state.get('bf_app_key_input', '')
        if not all([username, password, app_key]):
            print("[BETFAIR] ❌ Credenciais ausentes (preencha no painel ou nos secrets)")
            return False

        # Tenta login via cert se disponível
        cert_pem = st.secrets.get("BF_CERT_PEM", "") if "BF_CERT_PEM" in st.secrets else ""
        cert_key = st.secrets.get("BF_CERT_KEY", "") if "BF_CERT_KEY" in st.secrets else ""
        if cert_pem and cert_key:
            ...
            with open(pem_path, 'wb') as f: f.write(base64.b64decode(cert_pem))
            with open(key_path, 'wb') as f: f.write(base64.b64decode(cert_key))
            ...
            # Login interativo via REST (sem certificados)
            session_token = None
            for url_base in ["https://identitysso.betfair.com", "https://identitysso.betfair.com.br"]:
                try:
                    resp = requests.post(
                        f"{url_base}/api/login",
                    ...
                    if data.get("status") == "SUCCESS":
                        ...
                        print(f"[BETFAIR] ✅ Login OK via {url_base}")
                        break
                except: continue
            if not session_token:
                print(f"[BETFAIR] ❌ Login falhou em todos os endpoints")
                return False
            client = betfairlightweight.APIClient(
                username=username, password=password, app_key=app_key,
                lightweight=True
        ...
        try:
            ...
            if isinstance(funds, dict):
                saldo = funds.get("availableToBetBalance", 0)
            else:
                saldo = getattr(funds, 'available_to_bet_balance', 0)
            st.session_state['bf_saldo'] = float(saldo)
            print(f"[BETFAIR] 💰 Saldo: R$ {saldo:.2f}")
        except Exception as e:
            print(f"[BETFAIR] ⚠️ Não conseguiu buscar saldo: {e}")
...
if BF_AVAILABLE:
    try:
        has_secrets = "BF_USERNAME" in st.secrets and "BF_PASSWORD" in st.secrets and "BF_APP_KEY" in st.secrets
        if has_secrets and not st.session_state.get('bf_ativo'):
            bf_login()
...
with st.sidebar:
    ...
    with st.expander("⚙️ Configurações", expanded=True):
        ...
        if BF_AVAILABLE:
            ...
            # Credentials (usa secrets ou inputs manuais)
            bf_user_default = st.secrets.get("BF_USERNAME", "") if "BF_USERNAME" in st.secrets else st.session_state.get('bf_username_input', '')
            bf_pass_default = st.secrets.get("BF_PASSWORD", "") if "BF_PASSWORD" in st.secrets else st.session_state.get('bf_password_input', '')
            bf_key_default = st.secrets.get("BF_APP_KEY", "") if "BF_APP_KEY" in st.secrets else st.session_state.get('bf_app_key_input', '')
            if not ("BF_USERNAME" in st.secrets and "BF_PASSWORD" in st.secrets and "BF_APP_KEY" in st.secrets):
                st.session_state['bf_username_input'] = st.text_input("BF Usuário:", value=bf_user_default, type="password")
                st.session_state['bf_password_input'] = st.text_input("BF Senha:", value=bf_pass_default, type="password")
                st.session_state['bf_app_key_input'] = st.text_input("BF App Key:", value=bf_key_default, type="password")
            ...
            if not st.session_state.get('bf_ativo'):
                if st.button("🔗 Conectar Betfair"):
                    with st.spinner("Conectando..."):
                        if bf_login(): st.success("✅ Betfair conectada!")
                        else: st.error("❌ Falha no login. Verifique credenciais.")
            else:
                st.markdown('<span style="color:lime">✅ Betfair Conectada</span>', unsafe_allow_html=True)
