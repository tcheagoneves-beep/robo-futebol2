...
def bf_login():
    ...
    try:
        ...
        if cert_crt and cert_key_pem and "BEGIN" in cert_crt:
            try:
                import tempfile
                ...
                if st.button("🔗 Conectar Betfair"):
                    with st.spinner("Conectando à Betfair..."):
                        ...
                        if ok:
                            ...
                            st.error("❌ Falha no login. Verifique os logs no terminal (Manage app → Logs).")
                            st.caption("Dicas: verifique se USERNAME é o email, PASSWORD a senha da Betfair, e APP_KEY a chave da aplicação.")
            else:
