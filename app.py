...
- {txt_pessoal}
        ...
        if jogos_encontrados and len(jogos_encontrados) > 0:
            # Para cada jogo recomendado, faz análise completa
            texto_ia += "\n\n━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            texto_ia += "🧠 <b>ANÁLISE DETALHADA (IA COMPLETA)</b>\n"
            texto_ia += "━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            
            for jogo_nome in jogos_encontrados[:3]:  # Top 3 jogos
                try:
                    # Extrai times
                    if ' x ' in jogo_nome:
                        ...
                        if '(' in time_fora:
                            time_fora = time_fora.split('(')[0].strip()
                        
                        # Busca odd
                        jogo_key = f"{time_casa} x {time_fora}"
                        if jogo_key in mapa_jogos:
                            fid_jogo = mapa_jogos[jogo_key]
                            ...
                            if odd_jogo > 0:
                                # Análise completa
                                analise_pre = ia_analise_completa(
                                    ...
                                    odd_atual=odd_jogo,
                                    probabilidade_ia=75,
                                    stats_jogo="Análise Macro/Micro",
                                    big_data_info=""
                                )
                                
                                texto_ia += f"\n\n⚽ <b>{jogo_nome}</b>\n"
                                texto_ia += formatar_mensagem_ia_completa(
                                    analise_pre, 
                                    tipo_sinal="PRE_JOGO"
                                )
                except Exception as e:
                    print(f"Erro análise individual Sniper: {e}")
                    continue
...
def enviar_multipla_matinal(token, chat_ids, api_key):
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # [NOVA IA COMPLETA] Análise da múltipla como um todo
    # ═══════════════════════════════════════════════════════════════════════
    ...
    if IA_ATIVADA and len(jogos) > 0:
        try:
            ...
            analise_mult = ia_analise_completa(
                ...
                stats_jogo=f"{len(jogos)} jogos selecionados",
                big_data_info=""
            )
...
def enviar_alerta_alternativos(token, chat_ids, api_key):
    ...
    for s in sinais:
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # [NOVA IA COMPLETA] Análise para mercados alternativos
        # ═══════════════════════════════════════════════════════════════════════
        ...
        if IA_ATIVADA:
            try:
                ...
                if ' x ' in s['jogo']:
                    ...
                    analise_alt = ia_analise_completa(
                        estrategia=s['titulo'],
