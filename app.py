...
def analisar_financeiro_com_ia(stake_padrao, banca_inicial):
    ...
    try:
        ...
        # [FIX] Filtra apenas sinais APROVADOS pela IA (banca real = só o que entrou)
        if 'Opiniao_IA' in df_hoje.columns:
            df_aprovados = df_hoje[df_hoje['Opiniao_IA'] == 'Aprovado']
            df_arriscados = df_hoje[df_hoje['Opiniao_IA'].isin(['Arriscado', 'Neutro'])]
        else:
            df_aprovados = df_hoje
            df_arriscados = pd.DataFrame()

        # Filtra para 1 entrada por jogo (evita duplicidade no mesmo jogo)

        df_hoje = df_aprovados.drop_duplicates(subset=['FID'])
        ...
        # Conta arriscados que deram green/red (para info extra)
        arr_greens = len(df_arriscados[df_arriscados['Resultado'].str.contains('GREEN', na=False)]) if not df_arriscados.empty else 0
        arr_reds = len(df_arriscados[df_arriscados['Resultado'].str.contains('RED', na=False)]) if not df_arriscados.empty else 0
...
<i>*Cenário: Apenas sinais APROVADOS pela IA.</i>

📋 <b>SINAIS IGNORADOS (Arriscados):</b>
🟢 {arr_greens} teriam sido GREEN | 🔴 {arr_reds} teriam sido RED
<i>Esses sinais foram enviados mas marcados como "Arriscado" pela IA.</i>
    ...
    if 6 <= agora.hour < 12:  # [FIX] Matinal estendido até 12h
        ...
        if not st.session_state['matinal_enviado']:
            try:
                ...
                print(f"[SNIPER MATINAL] Resultado: {str(insights)[:100]}...")
                bloqueios = ["Sem jogos", "Erro", "Nenhum jogo", "IA Offline", "dados suficientes"]
                ...
                if tem_conteudo:
                    ...
                    try: salvar_snipers_do_texto(insights)
                    except: pass
                    print("[SNIPER MATINAL] ✅ Enviado!")
                else:
        ...
        if not st.session_state.get('sniper_tarde_enviado', False):
            try:
                ...
                if tem_conteudo:
                    ...
                    print(f"[SNIPER TARDE] ✅ Enviado com sucesso!")
                else:
            ...
            if res_final:
                ...
                try:
                    ...
                    if key_apura not in st.session_state['banca_updates']:
                        ...
                        try:
                            raw = str(s.get('Stake_Recomendado_RS', '')).strip()
                            if raw:
                                ...
                                if clean:
                                    parsed = float(clean)
                                    if 0 < parsed < 50000: stake_val = parsed
                        except: pass
                ...
                if deve_enviar:
                    ...
                    if tipo_msg == "GREEN" and key_green not in st.session_state['alertas_enviados']:
                         try:
                            stake_g = float(st.session_state.get('stake_padrao', 10.0))
                            try:
                                raw_stake_g = str(s.get('Stake_Recomendado_RS', '')).strip()
                                if raw_stake_g:
                                    # Remove R$, espaços, e converte vírgula
                                    clean = raw_stake_g.replace('R$', '').replace(' ', '').strip()
                                    # Se tem ponto E vírgula, remove ponto (milhar) e converte vírgula
                                    if '.' in clean and ',' in clean:
                                        clean = clean.replace('.', '').replace(',', '.')
                                    elif ',' in clean:
                                        clean = clean.replace(',', '.')
                         ...
                         try:
                            stake_r = float(st.session_state.get('stake_padrao', 10.0))
                            try:
                                raw_stake_r = str(s.get('Stake_Recomendado_RS', '')).strip()
                                if raw_stake_r:
                                    clean = raw_stake_r.replace('R$', '').replace(' ', '').strip()
                                    if '.' in clean and ',' in clean:
                                        clean = clean.replace('.', '').replace(',', '.')
                                    elif ',' in clean:
                                        clean = clean.replace(',', '.')
                    ...
                    for s in lista_sinais:
                        ...
                        try:
                            ...
                            if prob_k > 0 and odd_val > 1.0:
                                banca_k = float(st.session_state.get('banca_atual', st.session_state.get('banca_inicial', 1000.0)))
                                kelly_m = st.session_state.get('kelly_modo', 'fracionario')
                                kelly_r = calcular_stake_recomendado(banca_k, prob_k, odd_val, kelly_m)
                        ...
                        if adicionar_historico(item):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if prob_num_kelly > 0 and odd_val > 1.0:
                                        banca_k = float(st.session_state.get('banca_atual', st.session_state.get('banca_inicial', 1000.0)))
                                        kelly_modo = st.session_state.get('kelly_modo', 'fracionario')
                                        kelly_res = calcular_stake_recomendado(banca_k, prob_num_kelly, odd_val, kelly_modo)
                                        if kelly_res and kelly_res['valor'] > 0:
                                            kelly_valor_sinal = kelly_res['valor']
                                            retorno_potencial = round(kelly_valor_sinal * (odd_val - 1), 2)
                                            kelly_info = f"├─ 💰 <b>Apostar:</b> R$ {kelly_valor_sinal:.2f} ({kelly_res['porcentagem']:.1f}%) → Retorno: +R$ {retorno_potencial:.2f}\n"
                                except: pass
                                # Fallback: se Kelly falhou, mostra stake padrão
                                if not kelly_info:
                                    sp = float(st.session_state.get('stake_padrao', 10.0))
                                    retorno_p = round(sp * (odd_val - 1), 2) if odd_val > 1 else 0
                                    kelly_info = f"├─ 💰 <b>Apostar:</b> R$ {sp:.2f} (padrão) → Retorno: +R$ {retorno_p:.2f}\n"
