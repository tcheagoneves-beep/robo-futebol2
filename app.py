...
if 'matinal_retry' not in st.session_state: st.session_state['matinal_retry'] = 0
...
def verificar_reset_diario():
    ...
    if st.session_state['data_api_usage'] != hoje_utc:
        ...
        st.session_state['matinal_retry'] = 0
...
def _validar_palpite_sniper(jogo_nome, tipo_zona, mapa_jogos, api_key):
    ...
    """
    score = 65  # Base positiva (Gemini já selecionou com dados)
    motivos = []
    ...
    if not jogo_info:
        return 65, "sem_match"  # Não achou no mapa, mas Gemini selecionou → passa
...
def gerar_insights_matinais_ia(api_key):
    ...
    try:
        ...
        for j in jogos_candidatos:
            if count >= 15: break  # [OTIMIZADO] Max 15 jogos (economiza ~250 API calls)
            ...
            # [MELHORIA V2] WINRATE PESSOAL (ambos os times)
            txt_pessoal = ''
            try:
                df_sheets = st.session_state.get('historico_full', pd.DataFrame())
            ...
            # 1. Referência de Preço → REMOVIDO do loop (gastava 15 API calls desnecessárias)
            # Odd só é usada como informação, não bloqueia nenhum jogo
            odd_val = 0
            ...
            # 4. [NOVO] BIG DATA FIREBASE (1100+ jogos)
            txt_bigdata = ""
            try:
                bd = consultar_bigdata_cenario_completo(home_id, away_id)
                if bd and "Sem dados" not in str(bd) and "Offline" not in str(bd):
            ...
            # 5. [NOVO] WINRATE POR LIGA (filtra ligas tóxicas)
            txt_wr_liga = ""
            try:
                df_sheets = st.session_state.get('historico_full', pd.DataFrame())
                if df_sheets is not None and not df_sheets.empty:
                    ...
                    if len(df_liga_done) >= 3:
                        ...
                        txt_wr_liga = f"\n            📊 WINRATE LIGA ({liga}): {wr_liga:.0f}% em {len(df_liga_done)} jogos"
                        if wr_liga < 40:
                            txt_wr_liga += " ⚠️ LIGA TÓXICA"
            except: pass

            # 6. [NOVO] Aprendizado automático (ligas bloqueadas)
            txt_regra = ""
            try:
                regras = st.session_state.get('regras_aprendidas', {})
                ...
                for strat, ligas_ruins in ligas_block_all.items():
                    if liga in ligas_ruins:
                        txt_regra = f"\n            🚫 ALERTA APRENDIZADO: Liga bloqueada para {strat}"
                        break
            except: pass
        ...
        if not lista_para_ia: return "Nenhum dado disponível para análise.", {}
        ...
        🚨 REGRAS OBRIGATÓRIAS:
        - Se um jogo tem "⚠️ LIGA TÓXICA" ou "🚫 ALERTA APRENDIZADO" → NÃO indique esse jogo.
        - Use os dados do BIG DATA (🗄️) para cruzar com os 50 jogos. Se ambos concordam → Confiança alta.
        ...
        2. ⚡ **GOLS (OVER):**
           - REGRA DE OURO: Escolha a linha com base nos DADOS, não no palpite.
           - Se Over 1.5 FT > 75% nos DOIS times → Indique "Mais de 1.5 Gols" (MAIS SEGURO)
           - Se Over 2.5 FT > 65% nos DOIS times → Pode indicar "Mais de 2.5 Gols"
           - Se Over 2.5 FT < 55% em QUALQUER time → NÃO indique Over 2.5 (use 1.5)
           - Se Over 0.5 HT > 65% nos DOIS times → Pode indicar "Gol no 1º Tempo"
           - PREFIRA SEMPRE a linha MAIS SEGURA (Over 1.5 > Over 2.5)
        ...
        🏆 Liga: [Nome da liga]
        🎯 Palpite: **MAIS de 1.5 Gols** (ou "Mais de 2.5" APENAS se Over 2.5 FT > 65% nos dois times)
        💰 Ref (Over 1.5): @[valor da odd]
        📝 Motivo: [Cite dados de 50 jogos - Over 1.5: X%, Over 2.5: Y%]
        ...
        response = gemini_safe_call(prompt, generation_config=genai.types.GenerationConfig(temperature=0.0))
        if not response:
            return "Erro: Gemini 429 (limite atingido)", {}
        ...
        # Melhora estrutura visual (SEM criar tags HTML abertas)
        ...
        # [SAFETY] Fecha qualquer tag HTML aberta (Telegram rejeita HTML malformado)
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # [NOVO] VALIDAÇÃO INTELIGENTE PÓS-GEMINI
        # Cruza CADA palpite do Gemini contra dados REAIS antes de enviar
        # ═══════════════════════════════════════════════════════════════════════
        try:
            texto_validado = validar_snipers_pos_gemini(texto_ia, mapa_jogos, api_key)
            # Fallback: se validação removeu tudo, enviar original
            if texto_validado and len(texto_validado) > 100:
                texto_ia = texto_validado
            else:
                print("[SNIPER] ⚠️ Validação esvaziou texto, enviando original")
        except Exception as e:
            print(f"[SNIPER] ⚠️ Validação pós-Gemini falhou: {e} (enviando sem validação)")
...
def verificar_alerta_matinal(token, chat_ids, api_key):
    ...
    if 6 <= agora.hour < 12:  # [FIX] Matinal estendido até 12h
        ...
        if not st.session_state['matinal_enviado']:
            # ═══ [V24 APPROACH] Envio SIMPLES e DIRETO (comprovado) ═══
            ...
            mapa_jogos = result_matinal[1] if isinstance(result_matinal, tuple) and len(result_matinal) > 1 else {}
            print(f"[SNIPER MATINAL] Resultado: {len(str(insights))} chars")

            # Detecta ERRO (429, timeout, etc) → NÃO marca como enviado, tenta de novo
            eh_erro = "Erro" in str(insights)[:10] or "429" in str(insights) or "Resource" in str(insights)

            # Controle de retries para não ficar em loop infinito
            retry_count = st.session_state.get('matinal_retry', 0)

            if eh_erro and retry_count < 3:
                st.session_state['matinal_retry'] = retry_count + 1
                print(f"[SNIPER MATINAL] ❌ Erro temporário (tentativa {retry_count+1}/3), retry no próximo ciclo")
                # NÃO marca matinal_enviado = True → tenta de novo
            elif eh_erro:
                print(f"[SNIPER MATINAL] ❌ Erro persistente após 3 tentativas, desistindo")
                st.session_state['matinal_enviado'] = True
            elif insights and "Sem jogos" not in str(insights) and "IA Offline" not in str(insights) and "Nenhum dado" not in str(insights):
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
                msg_final = f"🌅 <b>SNIPER MATINAL (IA + DADOS)</b>\n\n{insights}"
                for cid in ids:
                    enviar_telegram(token, cid, msg_final)
                print(f"[SNIPER MATINAL] ✅ Enviado para {len(ids)} chats!")
                try: salvar_snipers_completo(str(insights), mapa_jogos, "Matinal")
                except: pass
                st.session_state['matinal_enviado'] = True  # Só marca quando REALMENTE enviou
            else:
                print(f"[SNIPER MATINAL] ⚠️ Sem conteúdo válido")
                st.session_state['matinal_enviado'] = True  # Sem jogos = ok, não tenta de novo
    ...
    if 12 <= agora.hour < 15:  # Janela estendida até 15h
        if not st.session_state.get('sniper_tarde_enviado', False):
            ...
            mapa_jogos_t = result_tarde[1] if isinstance(result_tarde, tuple) and len(result_tarde) > 1 else {}
            eh_erro_t = "Erro" in str(insights_tarde)[:10] or "429" in str(insights_tarde)
            if eh_erro_t:
                print(f"[SNIPER TARDE] ❌ Erro temporário, retry no próximo ciclo")
            elif insights_tarde and "Sem jogos" not in str(insights_tarde) and "IA Offline" not in str(insights_tarde):
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
                msg_tarde = f"☀️ <b>SNIPER DA TARDE (IA + DADOS)</b>\n\n{insights_tarde}"
                for cid in ids: enviar_telegram(token, cid, msg_tarde)
                try: salvar_snipers_completo(str(insights_tarde), mapa_jogos_t, "Tarde")
                except: pass
    ...
    if 17 <= agora.hour < 18:
        if not st.session_state.get('sniper_noite_enviado', False):
            ...
            mapa_jogos_n = result_noite[1] if isinstance(result_noite, tuple) and len(result_noite) > 1 else {}
            eh_erro_n = "Erro" in str(insights_noite)[:10] or "429" in str(insights_noite)
            if eh_erro_n:
                print(f"[SNIPER NOITE] ❌ Erro temporário, retry no próximo ciclo")
            elif insights_noite and "Sem jogos" not in str(insights_noite) and "IA Offline" not in str(insights_noite):
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
                msg_noite = f"🌙 <b>SNIPER DA NOITE (IA + DADOS)</b>\n\n{insights_noite}"
                for cid in ids: enviar_telegram(token, cid, msg_noite)
                try: salvar_snipers_completo(str(insights_noite), mapa_jogos_n, "Noite")
                except: pass
...
def conferir_resultados_sniper(jogos_live, api_key):
    ...
    snipers = [s for s in hist if "Sniper" in s.get('Estrategia', '') and s.get('Resultado') == "Pendente"]
    ...
    for s in snipers:
        ...
        tipo = s.get('Tipo_Apuracao', '')
        palpite = s.get('Palpite_Original', '').lower()
        res_final = None

        # ═══ APURAÇÃO POR TIPO ═══
        if tipo == 'OVER' or 'gol' in s.get('Estrategia', '').lower():
        ...
        elif tipo == 'UNDER' or 'under' in s.get('Estrategia', '').lower():
            linha_match = re.search(r'(\d+\.?\d*)\s*gol', palpite)
            if linha_match:
                linha = float(linha_match.group(1))
                res_final = '✅ GREEN' if total_gols < linha else '❌ RED'
        ...
        elif tipo == 'RESULTADO' or 'match' in s.get('Estrategia', '').lower():
