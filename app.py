...
def gerar_analise_mercados_alternativos_ia(api_key):
    ...
    try:
        ...
        for j in jogos_candidatos:
            # [CORRIGIDO v7.2] Reduzido de 30 para 10 (economiza ~460 API calls)
            ...
            media_escanteios_casa = 0
            media_escanteios_fora = 0
            if stats:
                media_cartoes_total = stats['home']['avg_cards'] + stats['away']['avg_cards']
                # [NOVO v7.2] Estimar escanteios via Over 2.5 (times com muitos gols = muitos ataques = muitos escanteios)
                over_pct_h = stats['home'].get('micro', 0) if isinstance(stats['home'].get('micro'), (int, float)) else 60
                over_pct_a = stats['away'].get('micro', 0) if isinstance(stats['away'].get('micro'), (int, float)) else 60
        ...
        1. 🟨 **MERCADO DE CARTÕES (Over e Under):**
           - **MODO AÇOUGUEIRO (OVER):** Média Soma > 4.5 e/ou Juiz rigoroso → "Mais de 3.5" ou "Mais de 4.5"
           - **MODO JOGO LIMPO (UNDER):** Média Soma < 3.5 → "Menos de 4.5" ou "Menos de 3.5"
           - OBRIGATÓRIO: Dê a linha exata.
        
        2. 🧤 **DEFESAS DO GOLEIRO (MURALHA):**
        ...
        3. 🎯 **FINALIZAÇÕES (CHUTES NO GOL):**
           - Só se "Permite Jogador? SIM".
           - Favorito com Odd < 1.70 → Atacante/Meia ofensivo Over 1.5 Chutes no Gol.
        
        4. ⛳ **ESCANTEIOS (NOVO):**
           - **ESCANTEIOS OVER:** Se o cenário é "Favorito Claro" ou "Massacre" → Favorito geralmente pressiona mais → "Mais de 4.5 Escanteios" para o time favorito, ou "Mais de 8.5 Escanteios na Partida".
           - **ESCANTEIOS UNDER:** Se o cenário é "Equilibrado" e ambas as odds altas → jogo pode ser mais travado → "Menos de 9.5 Escanteios na Partida".
           - OBRIGATÓRIO: Dê a linha exata e especifique se é para o time ou para a partida.
        ...
        {{
            "sinais": [
                {{
                    ...
                    "destaque": "Análise detalhada com dados que justificam a indicação.",
                    "indicacao": "Mais de 4.5 Cartões" ou "Goleiro do Time X Over 3.5 Defesas" ou "Mais de 9.5 Escanteios na Partida" etc.
                }}
            ]
        }}
        ...
        - Inclua pelo menos 1 sinal de ESCANTEIO.
        - Inclua pelo menos 1 sinal de CARTÕES.
        - Cada sinal deve ter um jogo DIFERENTE.
        - O FID deve ser exatamente o ID do jogo entre colchetes.
        """
