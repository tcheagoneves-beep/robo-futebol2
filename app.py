...
def classificar_tipo_estrategia(estrategia: str) -> str:
    ...
    # ─────────────────────────────────────────────────────────────────────
    # [NOVO v7.2] ESTRATÉGIAS DE ESCANTEIO
    # ─────────────────────────────────────────────────────────────────────
    escanteio_keywords = [
        'pressão sufocante',    # ⛳ Pressão Sufocante (Live)
        'pressao sufocante',    # (sem acento)
        'escanteio quente',     # ⛳🔥 Escanteio Quente (Live)
        'escanteio',            # (genérico)
    ...
    # 1.5 [NOVO] Checa ESCANTEIO antes de OVER (senão "escanteio" pode dar match com "over" genérico)
    for keyword in escanteio_keywords:
        if keyword in estrategia_lower:
            return 'ESCANTEIO'
...
def obter_descricao_aposta(estrategia: str) -> dict:
    ...
    # ─── [NOVO v7.2] ESCANTEIO STRATEGIES ───
    
    elif 'pressão sufocante' in estrategia_lower or 'pressao sufocante' in estrategia_lower:
        return {
            'tipo': 'ESCANTEIO',
            'aposta': 'Over Escanteios na Partida',
            'ordem': '👉 FAZER: Over Escanteios\n⛳ Aposta: Mais de X.5 Escanteios',
            'ganha_se': 'Total de escanteios SUPERA a linha',
            'perde_se': 'Total de escanteios FICA ABAIXO da linha'
        }
    
    elif 'escanteio quente' in estrategia_lower:
        return {
            'tipo': 'ESCANTEIO',
            'aposta': 'Over Escanteios Total na Partida',
            'ordem': '👉 FAZER: Over Escanteios Total\n⛳🔥 Aposta: Ritmo alto de escanteios',
            'ganha_se': 'Total de escanteios SUPERA a linha',
            'perde_se': 'Total de escanteios FICA ABAIXO da linha'
        }
    
    elif 'escanteio' in estrategia_lower or 'corner' in estrategia_lower:
        return {
            'tipo': 'ESCANTEIO',
            'aposta': 'Escanteios (Over/Under)',
            'ordem': '👉 FAZER: Apostar em Escanteios\n⛳ Aposta: Conforme indicação',
            'ganha_se': 'Meta de escanteios atingida',
            'perde_se': 'Meta de escanteios NÃO atingida'
        }
...
# [REMOVIDO v7.2] calcular_threshold_dinamico original (causava 100% veto)
# Definição correta (retorna 0) está após obter_odd_minima
...
ODD_MINIMA_POR_ESTRATEGIA = {
    ...
    "arame liso": 1.35,
    "under": 1.40,
    # [NOVO v7.2] Escanteios
    "pressão sufocante": 1.50,
    "pressao sufocante": 1.50,
    "escanteio quente": 1.50,
    "escanteio": 1.50,
}
...
def calcular_threshold_dinamico(estrategia, odd_atual):
    """
    [CORRIGIDO v7.1] Função que FALTAVA e causava 100% de vetos.
    Retorna 0 para NUNCA vetar automaticamente.
    """
    return 0
...
def verificar_mercados_bet365(api_key, fid):
    """
    [NOVO v7.2] Verifica quais mercados alternativos a Bet365 oferece para um jogo.
    Retorna dict: {'cartoes': True/False, 'escanteios': True/False, 'jogador': True/False}
    Usa 1 API call por jogo (sem filtro de bet = retorna TODOS os mercados).
    """
    resultado = {'cartoes': False, 'escanteios': False, 'jogador': False, 'gols': False, 'odd_casa': 0.0, 'odd_fora': 0.0}
    try:
        # Cache para não repetir chamadas
        cache_key = f"b365_mkts_{fid}"
        if cache_key in st.session_state:
            return st.session_state[cache_key]
        
        url = "https://v3.football.api-sports.io/odds"
        params = {"fixture": fid, "bookmaker": "8"}  # Bet365, SEM filtro de bet = retorna TUDO
        r = requests.get(url, headers={"x-apisports-key": api_key}, params=params).json()
        ...
        # Coletar TODOS os IDs e nomes de mercados disponíveis
        bet_ids = set()
        bet_names_lower = set()
        for b in bets:
            bet_ids.add(b.get('id', 0))
            bet_names_lower.add(str(b.get('name', '')).lower())
        
        # Debug
        print(f"[NEVES B365] FID {fid}: {len(bets)} mercados encontrados. IDs: {sorted(bet_ids)[:15]}")
        
        # === MAPEAR MERCADOS ===
        
        # Gols: ID 1 (Match Winner), 5 (Goals O/U), 8 (Both Teams Score)
        resultado['gols'] = bool({1, 5, 8} & bet_ids)
        
        # Cartões: ID 28 (Cards O/U), 115 (Booking Points), ou nome contém card/booking/cartão
        resultado['cartoes'] = bool(
            {28, 115, 100, 101, 102, 113, 114, 116} & bet_ids
            or any(w in n for n in bet_names_lower for w in ['card', 'booking', 'cartão', 'cartao'])
        )
        
        # Escanteios: ID 29 (Corners O/U), 45 (Corners 1H), ou nome contém corner/escanteio
        resultado['escanteios'] = bool(
            {29, 45, 46, 47, 48} & bet_ids
            or any(w in n for n in bet_names_lower for w in ['corner', 'escanteio'])
        )
        
        # Jogador (chutes, defesas, goleador): IDs variam muito, checar por nome
        resultado['jogador'] = bool(
            any(w in n for n in bet_names_lower 
                for w in ['player', 'shot', 'save', 'goalscorer', 'anytime', 'first goal', 'last goal',
                          'to score', 'jogador', 'chute', 'defesa'])
        )
        
        # [OTIMIZADO v7.2] Extrair odds Match Winner (evita segunda chamada de API)
        match_winner = next((b for b in bets if b.get('id') == 1), None)
        if match_winner:
            for v in match_winner.get('values', []):
                if v.get('value') == 'Home':
                    resultado['odd_casa'] = float(v.get('odd', 0))
                elif v.get('value') == 'Away':
                    resultado['odd_fora'] = float(v.get('odd', 0))
        
        st.session_state[cache_key] = resultado
        print(f"[NEVES B365] FID {fid}: C={resultado['cartoes']} E={resultado['escanteios']} J={resultado['jogador']} Odds={resultado['odd_casa']:.2f}/{resultado['odd_fora']:.2f}")
        
    except Exception as e:
        print(f"[NEVES B365] Erro verificar mercados {fid}: {e}")
    
    return resultado

def validar_sinal_bet365(api_key, sinal):
    """
    [NOVO v7.2] Validação FINAL antes de enviar: checa se o mercado específico
    existe na Bet365 para o FID do sinal. Retorna True se pode enviar.
    """
    try:
        fid = str(sinal.get('fid', '0'))
        if not fid or fid == '0': return True  # Sem FID = não filtra
        
        tipo = str(sinal.get('tipo', '')).upper()
        
        # Gols e genéricos sempre passam
        if tipo not in ('CARTAO', 'ESCANTEIO', 'GOLEIRO', 'CHUTE'):
            return True
        
        mercados = verificar_mercados_bet365(api_key, int(fid))
        
        if tipo == 'CARTAO' and not mercados['cartoes']:
            print(f"[NEVES B365] ❌ BLOQUEADO: Cartões não disponível na Bet365 para FID {fid}")
            return False
        elif tipo == 'ESCANTEIO' and not mercados['escanteios']:
            print(f"[NEVES B365] ❌ BLOQUEADO: Escanteios não disponível na Bet365 para FID {fid}")
            return False
        elif tipo in ('GOLEIRO', 'CHUTE') and not mercados['jogador']:
            print(f"[NEVES B365] ❌ BLOQUEADO: Jogador (chutes/defesas) não disponível na Bet365 para FID {fid}")
            return False
        
        return True
    except Exception as e:
        print(f"[NEVES B365] Erro validação: {e}")
...
def update_api_usage(headers):
    ...
    try:
        # [CORRIGIDO v7.2] Só atualiza se o header EXISTE (evita falso 100%)
        if 'x-ratelimit-requests-remaining' not in headers:
            return  # Não atualiza com dados fictícios
        limit = int(headers.get('x-ratelimit-requests-limit', 75000))
        remaining = int(headers.get('x-ratelimit-requests-remaining', limit))
        used = limit - remaining
        # Sanidade: se remaining > limit, algo está errado
        if used < 0: used = 0
...
def scanner_drop_odds_pre_live(api_key):
    ...
    try:
        # [OTIMIZADO v7.2] Usa cache compartilhado
        jogos = obter_fixtures_do_dia(api_key)
...
def adicionar_historico(item):
    ...
    try:
        ...
        # Mantém compatibilidade caso falte dado/coluna
...
def analisar_tendencia_50_jogos(api_key, home_id, away_id):
    """[OTIMIZADO v7.2] Cache em session_state para não repetir entre funções."""
    cache_key = f"t50_{home_id}_{away_id}"
    if cache_key in st.session_state:
        return st.session_state[cache_key]
    
    try:
        def get_stats_50(team_id):
            # Cache individual por time
            t_cache = f"t50_team_{team_id}"
            if t_cache in st.session_state:
                return st.session_state[t_cache]
            ...
            if not jogos: 
                result = {"qtd": 0, "win": 0, "draw": 0, "loss": 0, "over25": 0, "btts": 0}
                st.session_state[t_cache] = result
                return result
        ...
        resultado = {"home": get_stats_50(home_id), "away": get_stats_50(away_id)}
        st.session_state[cache_key] = resultado
        return resultado
    except: return None
...
def analisar_tendencia_macro_micro(api_key, home_id, away_id):
    """
    [OTIMIZADO v7.2] Versão LEVE: 2 API calls por jogo (1 por time).
    Cartões são estimados via team/statistics (1 call) em vez de 10 fixture/statistics (10 calls).
    Economia: de 22 calls/jogo para 2 calls/jogo = 90% de redução.
    """
    try:
        def get_team_stats_light(team_id):
            # Cache em session_state para compartilhar entre funções
            cache_key = f"team_light_{team_id}"
            if cache_key in st.session_state:
                return st.session_state[cache_key]
            ...
            for j in jogos[:5]: 
                ...
                if (j['goals']['home'] or 0) + (j['goals']['away'] or 0) >= 2: over_gols_count += 1

            pct_over_recent = int((over_gols_count / min(len(jogos), 5)) * 100)

            # --- PARTE 2: CARTÕES - ESTIMATIVA INTELIGENTE (0 API CALLS) ---
            # Estima cartões pelo tipo de jogo (derrotas = mais frustração = mais cartões)
            # Média geral da liga ~3.5 cartões por jogo, ajustado pelo perfil do time
            losses = sum(1 for j in jogos if (
                (j['teams']['home']['id'] == team_id and (j['goals']['home'] or 0) < (j['goals']['away'] or 0)) or
                (j['teams']['away']['id'] == team_id and (j['goals']['away'] or 0) < (j['goals']['home'] or 0))
            ))
            draws = sum(1 for j in jogos if (j['goals']['home'] or 0) == (j['goals']['away'] or 0))
            
            # Times que perdem mais → mais cartões (frustração)
            loss_ratio = losses / len(jogos)
            media_cards_estimada = 1.5 + (loss_ratio * 1.5) + (draws / len(jogos) * 0.3)
            
            total_vermelhos = max(0, int(losses * 0.15))  # ~15% das derrotas tem vermelho
            
            result = (resumo_txt, pct_over_recent, round(media_cards_estimada, 1), total_vermelhos)
            st.session_state[cache_key] = result
            return result

        # Processa Casa e Fora (2 API calls total)
        h_txt, h_pct, h_med_cards, h_reds = get_team_stats_light(home_id)
        a_txt, a_pct, a_med_cards, a_reds = get_team_stats_light(away_id)
        ...
        print(f"Erro MacroMicro Light: {e}")
        return None

def analisar_tendencia_macro_micro_HEAVY(api_key, home_id, away_id):
    """
    [VERSÃO PESADA] Só usa para ALTERNATIVOS (cartões precisos).
    22 API calls por jogo - usar com parcimônia!
    """
    # Cache agressivo
    cache_key = f"heavy_{home_id}_{away_id}"
    if cache_key in st.session_state:
        return st.session_state[cache_key]
    ...
    try:
        ...
        def get_team_stats_heavy(team_id):
            # Reutiliza cache light se existir
            light_cache = f"team_light_{team_id}"
            ...
            # PARTE 2: CARTÕES REAIS (10 API calls por time)
            total_amarelos = 0
            total_vermelhos = 0
            fids_para_buscar = [j['fixture']['id'] for j in jogos[:5]]  # Só 5 jogos (era 10)
            ...
            media_cards = total_amarelos / max(len(fids_para_buscar), 1)
            
            return resumo_txt, pct_over_recent, media_cards, total_vermelhos

        h_txt, h_pct, h_med_cards, h_reds = get_team_stats_heavy(home_id)
        a_txt, a_pct, a_med_cards, a_reds = get_team_stats_heavy(away_id)
        ...
        print(f"Erro MacroMicro Heavy: {e}")
        return None
...
def obter_fixtures_do_dia(api_key):
    """
    [NOVO v7.2] Cache CENTRAL de fixtures do dia. 
    Todas as funções matinais (sniper, multipla, alternativos, alavancagem) 
    usam esta ÚNICA função ao invés de cada uma fazer sua própria chamada.
    Economia: 4 API calls → 1 API call.
    """
    cache_key = "fixtures_dia_cache"
    cache_ts_key = "fixtures_dia_ts"
    
    agora = get_time_br()
    # Refresca a cada 30 minutos (ou na primeira chamada)
    cached_ts = st.session_state.get(cache_ts_key, 0)
    if cache_key in st.session_state and (agora.timestamp() - cached_ts) < 1800:
        return st.session_state[cache_key]
    ...
    try:
        ...
        res = requests.get(url, headers={"x-apisports-key": api_key}, params=params).json()
        jogos = res.get('response', [])
        st.session_state[cache_key] = jogos
        st.session_state[cache_ts_key] = agora.timestamp()
        print(f"[NEVES API] Fixtures do dia: {len(jogos)} jogos (1 API call)")
        return jogos
    except Exception as e:
        print(f"[NEVES API] Erro fixtures: {e}")
...
def gerar_multipla_matinal_ia(api_key):
    ...
    hoje = get_time_br().strftime('%Y-%m-%d')
    try:
        # [OTIMIZADO v7.2] Usa cache compartilhado (0 API calls extras)
        jogos = obter_fixtures_do_dia(api_key)
        ...
        for j in jogos_candidatos:
            # [OTIMIZADO v7.2] Reduzido de 30 para 15 jogos
            if count_validos >= 15: break
...
def gerar_insights_matinais_ia(api_key):
    ...
    hoje = get_time_br().strftime('%Y-%m-%d')
    try:
        # [OTIMIZADO v7.2] Usa cache compartilhado (0 API calls se já foi buscado)
        jogos = obter_fixtures_do_dia(api_key)
        ...
        for j in jogos_candidatos:
            # [OTIMIZADO v7.2] Reduzido de 80 para 25 jogos (economia ~70%)
            if count >= 25: break 
            ...
            # 1. Referência de Preço - FILTRO OBRIGATÓRIO BET365
            odd_val, odd_nome = buscar_odd_pre_match(api_key, fid)
            _bet365_ok = (odd_val > 0 and odd_val >= 1.01)
            
            # [CORRIGIDO v7.2] Se NÃO tem na Bet365, pula (não gasta API)
            if not _bet365_ok:
                continue
            ...
            if macro and micro:
                ...
                💰 Bet365 Confirmado: @{odd_val:.2f} ({odd_nome})
        ...
        ---------------------------------------------------------------------
        ✅ TODOS OS JOGOS ABAIXO ESTÃO DISPONÍVEIS NA BET365 (Verificado por API).
        ...
        SAÍDA OBRIGATÓRIA (VISUAL LIMPO - USE HTML):
        
        🔥 ZONA DE GOLS (OVER)
        ⚽ Jogo: [Nome] ([Liga])
        🎯 Palpite: MAIS de 2.5 Gols (ou Ambas Marcam / 1.5 HT)
        📝 Motivo: [Explique citando dados de 50 jogos]
        ...
        Ao final, escreva uma seção de observações CURTA (máximo 4 linhas) usando este formato visual:

        ✅ [Filtro aplicado]: [Breve explicação]
        ✅ [Filtro aplicado]: [Breve explicação]
        ⚠️ [Alerta se houver]: [Breve explicação]
        
        NÃO USE asteriscos (*) para formatar. Use APENAS emojis e texto limpo.
        NÃO repita "Lembre-se que estas são apenas sugestões..." ou disclaimers genéricos.
        As observações devem ser TÉCNICAS e CURTAS (1 linha cada).
        """
...
def gerar_analise_mercados_alternativos_ia(api_key):
    ...
    try:
        ...
        if not jogos_candidatos: 
            print("[NEVES ALT] Sem jogos NS para alternativos.")
            return []
        ...
        for j in jogos_candidatos:
            # [CORRIGIDO v7.2] Reduzido de 30 para 10 (economiza API calls)
            if count_validos >= 10: break 
            ...
            # [CORRIGIDO v7.2] Cache de macro_micro para não repetir chamadas
            cache_key_alt = f"ctx_{j['teams']['home']['id']}_{j['teams']['away']['id']}"
            if cache_key_alt in st.session_state:
                stats = st.session_state[cache_key_alt]
            else:
                stats = analisar_tendencia_macro_micro(api_key, j['teams']['home']['id'], j['teams']['away']['id'])
                if stats:
                    st.session_state[cache_key_alt] = stats
            ...
            # [NOVO v7.2] Verificar mercados disponíveis na Bet365 (1 API call, com cache)
            mercados_b365 = verificar_mercados_bet365(api_key, fid)
            
            if not mercados_b365['gols']:
                continue  # Bet365 nem tem odds para este jogo
            
            # [OTIMIZADO v7.2] Odds já extraídas pelo verificar_mercados (0 API calls extras)
            odd_casa = mercados_b365.get('odd_casa', 0)
            odd_fora = mercados_b365.get('odd_fora', 0)
            if odd_casa < 1.50: cenario = "Massacre Casa"
            elif odd_fora < 1.50: cenario = "Massacre Visitante"
            elif odd_casa < 2.00 or odd_fora < 2.00: cenario = "Favorito Claro"

            # [NOVO v7.2] String de mercados disponíveis para a IA
            mercados_str = []
            if mercados_b365['cartoes']: mercados_str.append("✅ CARTÕES")
            else: mercados_str.append("❌ CARTÕES")
            if mercados_b365['escanteios']: mercados_str.append("✅ ESCANTEIOS")
            else: mercados_str.append("❌ ESCANTEIOS")
            if mercados_b365['jogador']: mercados_str.append("✅ JOGADOR (Chutes/Defesas)")
            else: mercados_str.append("❌ JOGADOR")
            mercados_txt = " | ".join(mercados_str)

            # Manda para a IA se tiver dados úteis
            if media_cartoes_total > 0 or permite_player_props == "SIM" or cenario != "Equilibrado":
                dados_analise += f"""
                - Jogo: {home} x {away} ({liga_nome}) [FID: {fid}]
                  Juiz: {juiz} (Rigoroso? {juiz_rigor}) | Cenário: {cenario}
                  Odds: Casa @{odd_casa:.2f} | Fora @{odd_fora:.2f}
                  🏪 Bet365 Mercados: {mercados_txt}
                  Média Cartões (Soma): {media_cartoes_total:.1f}
                """
                count_validos += 1

        if not dados_analise: 
            print("[NEVES ALT] Nenhum jogo com dados suficientes para alternativos.")
            return []

        prompt = f"""
        ATUE COMO UM ESPECIALISTA EM MERCADOS ALTERNATIVOS DE FUTEBOL.
        
        Analise os jogos abaixo e encontre oportunidades em mercados especiais.
        
        ⚠️ REGRA CRÍTICA: Cada jogo tem uma linha "Bet365 Mercados" que mostra quais mercados
        a Bet365 REALMENTE oferece. Você SÓ pode sugerir um mercado se ele tiver ✅.
        Se o mercado tiver ❌, NÃO sugira esse tipo para esse jogo.
        
        DADOS DOS JOGOS:
        {dados_analise}
        
        SUA MISSÃO — ENCONTRAR 8 A 10 OPORTUNIDADES nos seguintes mercados
        (Geramos mais sinais porque alguns serão filtrados pela validação Bet365):
        
        1. 🟨 CARTÕES (Over e Under) — SÓ se "✅ CARTÕES":
           - AÇOUGUEIRO (OVER): Média Soma > 4.5 e/ou Juiz rigoroso → "Mais de 3.5" ou "Mais de 4.5"
           - JOGO LIMPO (UNDER): Média Soma < 3.5 → "Menos de 4.5" ou "Menos de 3.5"
           - OBRIGATÓRIO: Dê a linha exata.
        
        2. 🧤 DEFESAS DO GOLEIRO — SÓ se "✅ JOGADOR":
           - Cenário "Massacre Casa" → Goleiro Visitante Over 3.5 Defesas (time levando pressão = goleiro trabalha)
           - Cenário "Massacre Visitante" → Goleiro Casa Over 3.5 Defesas
           - Cenário "Favorito Claro" → Goleiro do time NÃO favorito Over 2.5 Defesas (linha mais baixa)
           - Indique: "Goleiro do [Time] Over X.5 Defesas" (NÃO nome do jogador, o nome do TIME)
        
        3. 🎯 FINALIZAÇÕES DO CENTROAVANTE — SÓ se "✅ JOGADOR":
           - Esta estratégia paga MUITO BEM nas casas de apostas.
           - Favorito com Odd < 1.80 → O centroavante/artilheiro do favorito finaliza muito.
           - Sugira: "[Nome do Atacante] Over 1.5 Chutes no Gol" ou "Over 2.5 Chutes no Gol"
           - Para artilheiros de elite (Haaland, Mbappé, Lewandowski, etc): "Over 2.5 Chutes no Gol"
           - Para atacantes regulares: "Over 1.5 Chutes no Gol"
           - OBRIGATÓRIO: Cite o NOME do jogador + linha exata.
           - O nome deve ser o centroavante/artilheiro TITULAR do time favorito.
        
        4. ⛳ ESCANTEIOS — SÓ se "✅ ESCANTEIOS":
           - OVER: Cenário "Favorito Claro"/"Massacre" → "Mais de 8.5 Escanteios na Partida"
           - UNDER: Cenário "Equilibrado" → "Menos de 9.5 Escanteios na Partida"
           - OBRIGATÓRIO: Linha exata + especifique partida ou time.
        ...
        {{
            "sinais": [
                {{
                    "fid": "ID_DO_JOGO", 
                    "tipo": "CARTAO" ou "GOLEIRO" ou "CHUTE" ou "ESCANTEIO",
                    "titulo": "🟨 AÇOUGUEIRO" ou "🕊️ JOGO LIMPO" ou "🧤 MURALHA" ou "🎯 SNIPER CHUTES" ou "⛳ ESCANTEIOS OVER" ou "⛳ ESCANTEIOS UNDER",
                    "jogo": "Time A x Time B",
                    "destaque": "Análise com dados que justificam. Para CHUTES: cite nome do atacante e por quê.",
                    "indicacao": "Haaland Over 2.5 Chutes no Gol" ou "Mais de 4.5 Cartões" ou "Goleiro do Juventus Over 3.5 Defesas" etc.
                }}
            ]
        }}
        
        REGRAS:
        - Gere entre 8 e 10 sinais (alguns serão filtrados se a Bet365 não oferecer o mercado).
        - NUNCA sugira um mercado com ❌ na Bet365 para aquele jogo.
        - OBRIGATÓRIO: Inclua pelo menos 1 sinal GOLEIRO (🧤 MURALHA) se algum jogo tiver ✅ JOGADOR e cenário de Massacre/Favorito.
        - OBRIGATÓRIO: Inclua pelo menos 1 sinal CHUTE (🎯 SNIPER CHUTES) se algum jogo tiver ✅ JOGADOR.
        - OBRIGATÓRIO: Inclua pelo menos 1 sinal CARTÃO (🟨 AÇOUGUEIRO ou 🕊️ JOGO LIMPO).
        - OBRIGATÓRIO: Inclua pelo menos 1 sinal ESCANTEIO (⛳).
        - Cada sinal deve ter um jogo DIFERENTE quando possível.
        - O FID deve ser EXATAMENTE o número entre colchetes.
        """
        
        response = model_ia.generate_content(prompt, generation_config=genai.types.GenerationConfig(response_mime_type="application/json"))
        st.session_state['gemini_usage']['used'] += 1
        resultado = json.loads(response.text).get('sinais', [])
        print(f"[NEVES ALT] Gerou {len(resultado)} sinais alternativos.")
        return resultado
    except Exception as e:
        print(f"[NEVES ALT] ERRO: {e}")
        return []
...
def consultar_ia_gemini(dados_jogo, estrategia, stats_raw, rh, ra, extra_context="", time_favoravel=""):
    if not IA_ATIVADA: return ("\n🤖 <b>IA LIVE:</b>\n⚠️ <b>SEM IA (Offline)</b>\n"
               "📝 <i>Analise os dados ao vivo acima e decida.</i>"), "N/A"
    try:
        ...
        # --- 2. ENGENHARIA DE DADOS (KPIs) --- [MOVIDO PARA CIMA - v7.2]
...
- Não ignore o placar atual.
        ...
        elif tipo_estrategia == 'ESCANTEIO':
            prompt += ("\n⛳ VOCÊ ESTÁ ANALISANDO UMA ESTRATÉGIA DE ESCANTEIOS:\n"
                      "- Aposta: VAI SAIR mais escanteios.\n"
                      "- APROVE se um time domina (posse alta, muitos ataques, muitos chutes) - isso gera escanteios.\n"
                      "- APROVE se ritmo de escanteios já é alto (ex: 5+ escanteios antes dos 40 min).\n"
                      "- VETE se jogo travado no meio campo, sem ataques, pouca posse de lado.\n"
                      "- Times favoritos jogando em casa tendem a forçar mais escanteios.\n"
                     )
...
# [MELHORIA V2] Regra explícita: Winrate Pessoal >=80% deve pesar como DIAMANTE
        ...
        if forca_veto:
            classe = 'ALTO RISCO'
            if prob_val > 30: prob_val = 30
            motivo = '⚠️ UNDER em jogo com {} gols (inconsistência) - CUIDADO'.format(gols_atuais)

        if odd_atual < odd_minima_necessaria:
            if not forca_aprovacao_minima:
                classe = 'ARRISCADO'
                motivo = '⚠️ Odd {:.2f} abaixo do ideal {:.2f} - AGUARDE VALORIZAR'.format(odd_atual, odd_minima_necessaria)
            else:
        ...
        if forca_aprovacao_minima:
            ...
            if classe in ['ALTO RISCO','ARRISCADO']:
                classe = 'PADRÃO'
        ...
        elif "ALTO RISCO" in classe: emoji = "🔴"; classe = "ALTO RISCO"
        elif "ARRISCADO" in classe: emoji = "⚠️"; classe = "ARRISCADO"
        elif prob_val < 35:
            emoji = "🔴"; classe = "ALTO RISCO"
        # Threshold dinâmico (nunca veta, apenas classifica)
        try:
            odd_local = float(dados_jogo.get('odd_atual', 1.50))
        except:
            odd_local = 1.50
        ...
        # [CORRIGIDO v7.2] Retorna análise genérica (NUNCA vazia!)
        print(f"Erro IA Gemini: {e}")
        return ("\n🤖 <b>IA LIVE (Flexível):</b>\n⚠️ <b>ARRISCADO (55%)</b>\n"
               "📊 <i>IA temporariamente indisponível</i>\n"
               "📝 <i>Dados ao vivo indicam oportunidade. Confira os stats acima e decida.</i>"), "55%"
    ...
    try:
        ...
        if tempo >= 75 and abs(gh - ga) <= 1:  # Início: 75 min (antes: 80)
            if total_fora <= 8 and ((rh >= 4) or (total_chutes_gol >= 5) or (ra >= 4)):  # Mais tolerante
                SINAIS.append({"tag": "💎 Sniper Final", "ordem": gerar_ordem_gol(total_gols, "Limite"), "stats": "Pontaria Ajustada", "rh": rh, "ra": ra, "favorito": "GOLS"})

        # ==============================================================================
        # [NOVO v7.2] ESTRATÉGIAS DE ESCANTEIOS (AO VIVO)
        # ==============================================================================
        total_escanteios = ck_h + ck_a
        
        # --- 12. PRESSÃO SUFOCANTE - Over Escanteios (Time Dominante) ---
        # Gatilho: Um time domina com muitos escanteios + posse + chutes
        if 30 <= tempo <= 75:
            # Casa pressionando
            if ck_h >= 4 and posse_h >= 55 and sog_h >= 2:
                linha_esc = total_escanteios + 1.5
                SINAIS.append({
                    "tag": "⛳ Pressão Sufocante", 
                    "ordem": f"👉 <b>FAZER:</b> Over Escanteios\n✅ Aposta: <b>Mais de {linha_esc:.1f} Escanteios na Partida</b>",
                    "stats": f"⛳ {ck_h}x{ck_a} Escanteios | 👟 {sog_h} Chutes no Gol (Casa)",
                    "rh": rh, "ra": ra, "favorito": "ESCANTEIOS"
                })
            # Visitante pressionando
            elif ck_a >= 4 and posse_a >= 55 and sog_a >= 2:
                linha_esc = total_escanteios + 1.5
                SINAIS.append({
                    "tag": "⛳ Pressão Sufocante", 
                    "ordem": f"👉 <b>FAZER:</b> Over Escanteios\n✅ Aposta: <b>Mais de {linha_esc:.1f} Escanteios na Partida</b>",
                    "stats": f"⛳ {ck_h}x{ck_a} Escanteios | 👟 {sog_a} Chutes no Gol (Fora)",
                    "rh": rh, "ra": ra, "favorito": "ESCANTEIOS"
                })
        
        # --- 13. ESCANTEIO QUENTE - Ritmo Alto de Escanteios ---
        # Gatilho: Muitos escanteios no total = ritmo alto → Over no total
        if 25 <= tempo <= 65:
            # Se já saíram muitos escanteios para o tempo de jogo (>= 0.15 por minuto)
            ritmo_esc = total_escanteios / max(tempo, 1)
            if total_escanteios >= 5 and ritmo_esc >= 0.13:
                projecao = int(ritmo_esc * 90)
                linha_esc_total = total_escanteios + 2.5
                SINAIS.append({
                    "tag": "⛳🔥 Escanteio Quente",
                    "ordem": f"👉 <b>FAZER:</b> Over Escanteios Total\n✅ Aposta: <b>Mais de {linha_esc_total:.1f} Escanteios na Partida</b>\n📊 <i>Projeção: ~{projecao} escanteios</i>",
                    "stats": f"⛳ {total_escanteios} Escanteios em {tempo}' (Ritmo: {ritmo_esc:.2f}/min)",
                    "rh": rh, "ra": ra, "favorito": "ESCANTEIOS"
                })
...
def atualizar_stats_em_paralelo(jogos_alvo, api_key):
    resultados = {}
    # [CORRIGIDO v7.2] Limita a 10 jogos por ciclo para economizar API
    jogos_alvo = jogos_alvo[:10]
    with ThreadPoolExecutor(max_workers=5) as executor:
...
def markdown_to_telegram_html(texto):
    """[NOVO v7.2] Converte markdown da IA para HTML do Telegram com formatação limpa."""
    if not texto: return texto
    import re as _re
    
    # 0. PRIMEIRO: Remove code fences (```html, ```json, ```, etc.)
    texto = _re.sub(r'```[a-zA-Z]*\s*\n?', '', texto)
    texto = _re.sub(r'```\s*', '', texto)
    
    # 1. Converter bullets markdown (* item) → emoji bullet (ANTES do bold/italic)
    ...
    # 3. *texto* → <i>texto</i> (mas não confundir com bullets já convertidos)
    texto = _re.sub(r'(?<!\w)\*([^*\n]+?)\*(?!\w)', r'<i>\1</i>', texto)
    
    # 4. __texto__ → <u>texto</u>
    texto = _re.sub(r'__(.+?)__', r'<u>\1</u>', texto)
    
    # 5. Limpa linhas de separação markdown (--- ou ***) → linha visual
    texto = _re.sub(r'\n\s*[-]{3,}\s*\n', '\n━━━━━━━━━━━━━━━━━━━━\n', texto)
    
    # 6. Remove # headers markdown → bold
    texto = _re.sub(r'^#{1,3}\s*(.+)$', r'<b>\1</b>', texto, flags=_re.MULTILINE)
    
    # 7. Remove disclaimers genéricos da IA (sempre os mesmos)
    disclaimers = [
        r'Lembre-se que estas são apenas sugestões.*?(?:\n|$)',
        r'A análise de um trader esportivo profissional.*?(?:\n|$)',
        r'Estas são apenas sugestões.*?(?:\n|$)',
        r'Aposte com responsabilidade.*?(?:\n|$)',
        r'Disclaimer:.*?(?:\n|$)',
    ]
    for d in disclaimers:
        texto = _re.sub(d, '', texto, flags=_re.IGNORECASE | _re.DOTALL)
    
    # 8. Limpa espaços duplos e linhas em branco excessivas
    texto = _re.sub(r'\n{3,}', '\n\n', texto)
    texto = _re.sub(r'[ \t]+\n', '\n', texto)
...
def enviar_telegram(token, chat_ids, msg):
    ...
    # [NOVO v7.2] Converte markdown → HTML automaticamente
    msg = markdown_to_telegram_html(msg)
...
def enviar_alerta_alternativos(token, chat_ids, api_key):
    ...
    print("[NEVES ALT] Iniciando geração de sinais alternativos...")
    sinais = gerar_analise_mercados_alternativos_ia(api_key)
    if not sinais: 
        print("[NEVES ALT] Nenhum sinal gerado pela IA.")
    ...
    for s in sinais:
        try:
            ...
            if len(str(s.get('destaque',''))) < 30:
                continue
            
            tipo = str(s.get('tipo', 'CARTAO')).upper()
            
            # ═══════════════════════════════════════════════════════
            # [NOVO v7.2] VALIDAÇÃO REAL BET365 - ANTES DE ENVIAR
            # Checa se o mercado REALMENTE existe na Bet365 para este jogo
            # Se não existir → PULA para o próximo sinal
            # ═══════════════════════════════════════════════════════
            if not validar_sinal_bet365(api_key, s):
                bloqueados += 1
                print(f"[NEVES ALT] Sinal bloqueado (sem mercado Bet365): {tipo} - {s.get('jogo','?')}")
                continue
            
            # [CORRIGIDO v7.2] Mensagem formatada por tipo
            if tipo == 'ESCANTEIO':
                msg = f"⛳ <b>{s['titulo']}</b>\n\n"
                msg += f"⚽ <b>{s['jogo']}</b>\n\n"
                msg += f"🔎 <b>Análise:</b>\n{s['destaque']}\n\n"
                ...
                msg += f"{'━' * 20}\n"
                msg += "📊 <i>Estratégia: Favoritos Agressivos (Escanteios)</i>\n"
                msg += "✅ <i>Mercado verificado na Bet365</i>"
            elif tipo == 'GOLEIRO':
                msg = f"🧤 <b>{s['titulo']}</b>\n\n"
                msg += f"⚽ <b>{s['jogo']}</b>\n\n"
                msg += f"🔎 <b>Análise:</b>\n{s['destaque']}\n\n"
                msg += f"🎯 <b>INDICAÇÃO:</b> {s['indicacao']}\n"
                msg += "⚠️ <i>Regra: Aposte no 'Goleiro do Time', não no nome do jogador.</i>\n"
                msg += "✅ <i>Mercado verificado na Bet365</i>"
            elif tipo == 'CHUTE':
                msg = f"🎯 <b>{s['titulo']}</b>\n\n"
                msg += f"⚽ <b>{s['jogo']}</b>\n\n"
            ...
            # Salvar no histórico
            linha_alvo = "0"
            try: linha_alvo = re.findall(r"[-+]?\d*\.\d+|\d+", str(s.get('indicacao','')))[0]
            except: pass
            
            # Estratégia legível para o histórico
            strat_nome = s.get('titulo', tipo)
            if 'ESCANTEIO' in tipo: strat_nome = f"⛳ Escanteios ({s.get('titulo', 'Over')})"
            
            item_alt = {
                "FID": f"ALT_{s.get('fid', '0')}", "Data": get_time_br().strftime('%Y-%m-%d'), 
                "Hora": get_time_br().strftime('%H:%M'),
                "Liga": "Mercado Alternativo", "Jogo": s.get('jogo', '?'), 
                "Placar_Sinal": f"Meta: {linha_alvo}",
                "Estrategia": strat_nome, "Resultado": "Pendente", 
            ...
            print(f"[NEVES ALT] Erro ao enviar sinal {s}: {e}")
    
    print(f"[NEVES ALT] Resultado: {enviados} enviados, {bloqueados} bloqueados (sem mercado Bet365)")
    st.session_state['alternativos_enviado'] = True
...
def verificar_alerta_matinal(token, chat_ids, api_key):
    agora = get_time_br()
    if 8 <= agora.hour < 11:
        # --- 1. SNIPER MATINAL (Análise de Jogos do Dia) ---
        if not st.session_state['matinal_enviado']:
            try:
                conteudo_ia, mapa_ids = gerar_insights_matinais_ia(api_key)
                # [CORRIGIDO v7.2] Filtro mais robusto - checa qualquer mensagem de "sem dados"
                tem_conteudo = (conteudo_ia 
                    and "Sem jogos" not in str(conteudo_ia) 
                    and "Nenhum jogo" not in str(conteudo_ia)
                    and "Erro" not in str(conteudo_ia)
                    and "Offline" not in str(conteudo_ia)
                    and len(str(conteudo_ia)) > 100)  # Conteúdo real tem >100 chars
                ...
                if tem_conteudo:
                    ...
                    print(f"Sniper Matinal: Aguardando dados ({agora.strftime('%H:%M')})")
            except Exception as e:
                print(f"Sniper Matinal erro: {e}")

        # --- 2. MÚLTIPLA MATINAL (depende do Sniper) ---
        if st.session_state['matinal_enviado'] and not st.session_state.get('multipla_matinal_enviada', False):
            try:
                ...
                print(f"Multipla matinal erro: {e}")
                st.session_state['multipla_matinal_enviada'] = True  # Não trava a cadeia
        
        # --- 3. MERCADOS ALTERNATIVOS - Cartões, Defesas, Finalizações ---
        # [CORRIGIDO v7.2] Roda independente do Sniper (a partir das 8:30)
        if agora.minute >= 30 or agora.hour >= 9:
            if not st.session_state.get('alternativos_enviado', False):
                try:
                    time.sleep(5); enviar_alerta_alternativos(token, chat_ids, api_key)
                except Exception as e:
                    print(f"Alternativos erro: {e}")
                    st.session_state['alternativos_enviado'] = True
        
        # --- 4. ALAVANCAGEM (Bet Builder) ---
        # [CORRIGIDO v7.2] Roda independente do Sniper (a partir das 10h)
...
def check_green_red_hibrido(jogos_live, token, chats, api_key):
    ...
    # [OTIMIZADO v7.2] Cache de status de fixtures para evitar API calls repetidas
    if 'fixture_status_cache' not in st.session_state:
        st.session_state['fixture_status_cache'] = {}
    cache_status = st.session_state['fixture_status_cache']
    agora_ts = time.time()
    ...
    for s in pendentes:
        ...
        # [OTIMIZADO v7.2] Primeiro tenta no mapa live (0 API calls)
        jogo_api = mapa_live.get(fid)
        
        if not jogo_api:
            # [OTIMIZADO v7.2] Checa cache antes de chamar API
            cached = cache_status.get(fid)
            if cached:
                last_check, cached_data = cached
                status_cached = cached_data['fixture']['status']['short'] if cached_data else 'NS'
                
                # Se game não começou (NS/TBD), não re-checa por 15 min
                if status_cached in ('NS', 'TBD', 'PST') and (agora_ts - last_check) < 900:
                    continue
                # Se game está ao vivo, re-checa a cada 3 min
                elif status_cached in ('1H', '2H', 'HT', 'ET') and (agora_ts - last_check) < 180:
                    jogo_api = cached_data
                # Se game terminou, usa o cache (não muda mais)
                elif status_cached in ('FT', 'AET', 'PEN', 'ABD'):
                    jogo_api = cached_data
                else:
                    # Re-checa
                    try:
                        ...
                        if res.get('response'):
                            ...
                            cache_status[fid] = (agora_ts, jogo_api)
                    except: pass
            else:
                # Primeira vez: checa uma vez e cacheia
                try:
                    res = requests.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": api_key}, params={"id": fid}).json()
                    if res.get('response'):
                        jogo_api = res['response'][0]
                        ...
                        cache_status[fid] = (agora_ts, None)
                        continue
                except: 
                    cache_status[fid] = (agora_ts, None)
                    continue
        ...
        if jogo_api:
            ...
            # [NOVO v7.2] ESCANTEIOS - Verifica total de escanteios vs linha
            elif tipo_estrategia == 'ESCANTEIO':
                if st_short in ['FT', 'AET', 'PEN', 'ABD', '2H'] or (jogo_api['fixture']['status'].get('elapsed', 0) or 0) >= 80:
                    try:
                        # Buscar stats para obter escanteios finais
                        stats_esc = st.session_state.get(f"st_{fid}", [])
                        if not stats_esc:
                            r_st = requests.get("https://v3.football.api-sports.io/fixtures/statistics", 
                                headers={"x-apisports-key": api_key}, params={"fixture": fid}).json()
                            stats_esc = r_st.get('response', [])
                        
                        if stats_esc and len(stats_esc) >= 2:
                            def gv_esc(l, t): return next((x['value'] or 0 for x in l if x['type'] == t), 0)
                            esc_h = gv_esc(stats_esc[0]['statistics'], 'Corner Kicks')
                            esc_a = gv_esc(stats_esc[1]['statistics'], 'Corner Kicks')
                            total_esc = esc_h + esc_a
                            
                            # Extrair linha do texto da aposta (ex: "Mais de 8.5")
                            import re as re_esc
                            linha_esc = 0
                            texto_sinal = str(s.get('Estrategia', '')) + ' ' + str(s.get('Placar_Sinal', ''))
                            nums = re_esc.findall(r"(\d+\.?\d*)\s*(?:Escanteio|escanteio|Corner)", texto_sinal)
                            if not nums:
                                nums = re_esc.findall(r"Mais de\s+(\d+\.?\d*)", texto_sinal)
                            if not nums:
                                nums = re_esc.findall(r"(\d+\.5)", texto_sinal)
                            if nums:
                                linha_esc = float(nums[0])
                            
                            if linha_esc > 0:
                                if total_esc > linha_esc:
                                    res_final = '✅ GREEN'
                                elif st_short in ['FT', 'AET', 'PEN', 'ABD']:
                                    res_final = '❌ RED'
                    except Exception as e_esc:
                        print(f"[NEVES] Erro apurar escanteios {fid}: {e_esc}")
                ...
                try:
                    ...
                    if key_apura not in st.session_state['banca_updates']:
                        ...
                        try:
                            ...
                            # [CORRIGIDO v7.2] O ponto É o decimal (formato Python). NÃO remover.
                            stake_str = stake_str.replace(',', '.')  # Só converte vírgula se vier do Sheets
                        ...
                        # [CORRIGIDO v7.2] Teto de segurança: stake máx 25% da banca
                        max_stake = saldo * 0.25
                        if stake_val > max_stake and max_stake > 0:
                            stake_val = max_stake
                        ...
                        # [CORRIGIDO v7.2] Banca nunca fica negativa
                        if saldo < 0:
                            saldo = 0.0
...
def conferir_resultados_sniper(jogos_live, api_key):
    ...
    # [OTIMIZADO v7.2] Cache compartilhado com check_green_red
    if 'fixture_status_cache' not in st.session_state:
        st.session_state['fixture_status_cache'] = {}
    cache_status = st.session_state['fixture_status_cache']
    agora_ts = time.time()
    ...
    for s in snipers:
        ...
        if not jogo:
            # [OTIMIZADO v7.2] Usa cache - não chama API a cada 60s para jogos que não começaram
            fid_int = int(fid) if fid.isdigit() else 0
            cached = cache_status.get(fid_int)
            if cached:
                last_check, cached_data = cached
                if cached_data:
                    status_cached = cached_data['fixture']['status']['short']
                    # Jogo não começou? Pula (re-checa a cada 15 min pelo check_green_red)
                    if status_cached in ('NS', 'TBD', 'PST'):
                        continue
                    # Jogo terminou? Usa cache
                    elif status_cached in ('FT', 'AET', 'PEN', 'INT', 'ABD'):
                        jogo = cached_data
                    # Jogo ao vivo? Re-checa só se cache > 3 min
                    elif (agora_ts - last_check) < 180:
                        jogo = cached_data
                    else:
                        try:
                            res = requests.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": api_key}, params={"id": fid}).json()
                            if res.get('response'): 
                                jogo = res['response'][0]
                                cache_status[fid_int] = (agora_ts, jogo)
                        except: pass
                else:
                    continue  # Fixture inválido no cache
            else:
                # Primeira vez: checa e cacheia
                try:
                    res = requests.get("https://v3.football.api-sports.io/fixtures", headers={"x-apisports-key": api_key}, params={"id": fid}).json()
                    if res.get('response'): 
                        jogo = res['response'][0]
...
def verificar_automacao_bi(token, chat_ids, stake_padrao):
    ...
    # [CORRIGIDO v7.2] Janela noturna: 22h-23h59 → envia relatórios automaticamente
    if agora.hour < 22:
        return  # Fora da janela noturna
    
    # --- 1. RELATÓRIO FINANCEIRO (Fechamento do Dia) ---
    if not st.session_state.get('financeiro_enviado', False):
        try:
            df_fin = st.session_state.get('historico_full', pd.DataFrame())
            if not df_fin.empty:
                df_hoje = df_fin[df_fin['Data'] == hoje_str].copy()
                df_resolvidos = df_hoje[df_hoje['Resultado'].str.contains('GREEN|RED', na=False, regex=True)]
                if len(df_resolvidos) >= 1:
                    greens = len(df_resolvidos[df_resolvidos['Resultado'].str.contains('GREEN', na=False)])
                    reds = len(df_resolvidos[df_resolvidos['Resultado'].str.contains('RED', na=False)])
                    total = len(df_resolvidos)
                    ...
                    for _, row in df_resolvidos.iterrows():
                        try:
                            odd_str = str(row.get('Odd', '1.50')).replace(',', '.')
                            odd_v = float(odd_str) if odd_str.replace('.','',1).isdigit() else 1.50
                            if 'GREEN' in str(row['Resultado']):
                                lucro_total += stake_padrao * (odd_v - 1)
                            else:
                                lucro_total -= stake_padrao
                        except:
                            pass
                    
                    roi = (lucro_total / (total * stake_padrao) * 100) if total > 0 else 0
                    emoji_resultado = "🟢" if lucro_total >= 0 else "🔴"
                    
                    # Top estratégias do dia
                    top_strats = ""
                    strat_counts = df_resolvidos.groupby('Estrategia').apply(
                        lambda x: f"{x['Resultado'].str.contains('GREEN', na=False).sum()}/{len(x)}"
                    )
                    for strat, score in strat_counts.items():
                        top_strats += f"  • {strat}: {score}\n"
                    
                    msg = (
                        f"🌙 <b>FECHAMENTO DO DIA</b>\n"
                        f"📅 {hoje_str}\n"
                        f"{'━' * 20}\n\n"
                        f"{emoji_resultado} <b>Resultado: R$ {lucro_total:+.2f}</b>\n"
                        f"📊 ROI: {roi:+.1f}%\n"
                        f"📈 Winrate: {wr:.0f}% ({greens}G / {reds}R)\n"
                        f"🎯 Total Entradas: {total}\n\n"
                        f"📋 <b>Por Estratégia:</b>\n{top_strats}\n"
                        f"💰 Stake usado: R$ {stake_padrao:.2f}\n"
                        f"{'━' * 20}\n"
                        f"{'🏆 Dia positivo! Banca protegida.' if lucro_total >= 0 else '⚠️ Dia negativo. Amanhã é novo dia.'}"
                    )
                    enviar_telegram(token, chat_ids, msg)
                    st.session_state['financeiro_enviado'] = True
                    print(f"[NEVES] Relatório Financeiro enviado: R$ {lucro_total:+.2f}")
        except Exception as e:
            print(f"[NEVES] Erro relatório financeiro: {e}")
    
    # --- 2. RELATÓRIO BI (Análise de Performance via IA) ---
    if not st.session_state.get('bi_enviado', False) and st.session_state.get('financeiro_enviado', False):
        try:
            if IA_ATIVADA:
                time.sleep(2)
                analise = analisar_bi_com_ia()
                if analise and "Sem dados" not in analise and "Amostra muito pequena" not in analise:
                    msg_bi = f"📊 <b>RELATÓRIO BI NOTURNO</b>\n\n{analise}"
                    enviar_telegram(token, chat_ids, msg_bi)
                    st.session_state['bi_enviado'] = True
                    print("[NEVES] Relatório BI enviado.")
                else:
                    st.session_state['bi_enviado'] = True  # Marca como enviado mesmo sem dados
        except Exception as e:
            print(f"[NEVES] Erro relatório BI: {e}")
    
    # --- 3. ANÁLISE DE ESTRATÉGIAS (Otimização via IA) ---
    if not st.session_state.get('ia_enviada', False) and st.session_state.get('bi_enviado', False):
        try:
            if IA_ATIVADA:
                time.sleep(2)
                otimizacao = otimizar_estrategias_existentes_ia()
                if otimizacao and "Sem dados" not in otimizacao:
                    msg_ia = f"🔧 <b>AJUSTE TÉCNICO (IA)</b>\n\n{otimizacao}"
                    enviar_telegram(token, chat_ids, msg_ia)
                    st.session_state['ia_enviada'] = True
                    print("[NEVES] Análise de estratégias enviada.")
                else:
                    st.session_state['ia_enviada'] = True
        except Exception as e:
            print(f"[NEVES] Erro análise estratégias: {e}")
...
if st.session_state.ROBO_LIGADO:
    with placeholder_root.container():
      try:
        carregar_tudo()
        verificar_reset_diario()  # [CORRIGIDO v7.2] Reseta contador diário ANTES de tudo
...
# ==============================================================================
        ...
        if not api_error:
            ...
            for j in jogos_live:
                ...
                if lista_sinais:
                    ...
                    # [CORRIGIDO v7.2] Cache de tendência (não muda durante o jogo, economiza ~22 API calls/jogo)
                    cache_key_ctx = f"ctx_{j['teams']['home']['id']}_{j['teams']['away']['id']}"
                    if cache_key_ctx not in st.session_state:
                        dados_contextuais = analisar_tendencia_macro_micro(safe_api, j['teams']['home']['id'], j['teams']['away']['id'])
                        st.session_state[cache_key_ctx] = dados_contextuais
                    else:
                        dados_contextuais = st.session_state[cache_key_ctx]
                    ...
                    # [CORRIGIDO v7.2] Cache de ratings (economiza 2+ API calls/jogo/ciclo)
                    cache_key_rating_h = f"rating_{j['teams']['home']['id']}"
                    cache_key_rating_a = f"rating_{j['teams']['away']['id']}"
                    if cache_key_rating_h not in st.session_state:
                        nota_home = buscar_rating_inteligente(safe_api, j['teams']['home']['id'])
                        st.session_state[cache_key_rating_h] = nota_home
                    else:
                        nota_home = st.session_state[cache_key_rating_h]
                    if cache_key_rating_a not in st.session_state:
                        nota_away = buscar_rating_inteligente(safe_api, j['teams']['away']['id'])
                        st.session_state[cache_key_rating_a] = nota_away
                    else:
                        nota_away = st.session_state[cache_key_rating_a]
                    ...
                    for s in lista_sinais:
                        ...
                        # [CORRIGIDO v7.2] Prioriza ordem DINÂMICA do sinal (ajustada ao placar atual)
                        texto_acao_original = s.get('ordem', desc_aposta['ordem'])
                        ...
                        elif desc_aposta['tipo'] == 'ESCANTEIO':
                            tipo_info = '⛳ <b>ESCANTEIOS</b>: Sai escanteio\n'
                        else:
                        ...
                        if IA_ATIVADA:
                            try:
                                ...
                                opiniao_lower = opiniao_txt.lower()
                                if "diamante" in opiniao_lower: 
                                    opiniao_db = "Aprovado"
                                elif "aprovado" in opiniao_lower or "padrão" in opiniao_lower or "padrao" in opiniao_lower: 
                                    opiniao_db = "Aprovado"
                                elif "alto risco" in opiniao_lower: 
                                    opiniao_db = "Alto Risco"
                                elif "arriscado" in opiniao_lower: 
                                    opiniao_db = "Arriscado"
                                else: 
                                    opiniao_db = "Aprovado"
                            except Exception as e_ia: 
                                print(f"Erro IA classify: {e_ia}")
                                opiniao_db = "Arriscado"
                        else:
                            opiniao_db = "Offline"

                        status_inicial = "Pendente"
                        # [CORRIGIDO v7.0] Nunca veta - apostador decide
                        ...
                        # [NOVO v7.2] Para escanteios, salva a linha no Placar_Sinal
                        if desc_aposta['tipo'] == 'ESCANTEIO':
                            import re as re_esc2
                            ordem_txt = str(s.get('ordem', ''))
                            nums_esc = re_esc2.findall(r"Mais de\s+(\d+\.?\d*)", ordem_txt)
                            if nums_esc:
                                item['Placar_Sinal'] = f"Mais de {nums_esc[0]} Escanteios"
                        ...
                        if adicionar_historico(item):
                            try:
                                ...
                                if not df_h.empty:
                                    ...
                                    # [CORRIGIDO v7.2] Só conta apostas resolvidas (GREEN/RED), ignora Pendente/VETADO
                                    strat_resolvidas = strat_f[strat_f['Resultado'].str.contains('GREEN|RED', na=False, regex=True)]
                                    if len(strat_resolvidas) >= 1:
                                        greens_s = len(strat_resolvidas[strat_resolvidas['Resultado'].str.contains('GREEN', na=False)])
                                        wr_s = (greens_s / len(strat_resolvidas)) * 100
                                ...
                                # [CORRIGIDO v7.2] SEMPRE envia - obs contextual
                                if opiniao_db == "Aprovado":
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"✅ Sinal Enviado: {s['tag']}")
                                elif "Alto Risco" in str(opiniao_db):
                                    msg += "\n🔴 <b>⚠️ ALTO RISCO - IA recomenda cautela.</b>"
                                    msg += "\n💡 <i>Decisão final é sua. Confira os dados acima.</i>"
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"🔴 Sinal Alto Risco Enviado: {s['tag']}")
                                elif opiniao_db == "Arriscado":
                                    msg += "\n👀 <i>Obs: IA detectou risco moderado. Confira os dados.</i>"
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"⚠️ Sinal Arriscado Enviado: {s['tag']}")
                                else:
                                    # Offline ou outro - envia sem obs negativa
                                    enviar_telegram(safe_token, safe_chat, msg)
                                    st.toast(f"📡 Sinal Enviado: {s['tag']}")
        ...
        _intervalo = INTERVALO if 'INTERVALO' in dir() else 60
        for i in range(_intervalo, 0, -1):
            st.markdown(f'<div class="footer-timer">Próxima varredura em {i}s</div>', unsafe_allow_html=True)
            time.sleep(1)
        st.rerun()
      except Exception as e_main:
        # [CORRIGIDO v7.2] Mostra erro em vez de tela preta
        import traceback
        erro_txt = traceback.format_exc()
        print(f"[NEVES ERRO PRINCIPAL] {e_main}\n{erro_txt[-500:]}")
        st.error(f"⚠️ Erro no ciclo: {type(e_main).__name__}: {e_main}")
        st.code(erro_txt[-800:], language="text")
        st.warning("🔄 Tentando novamente em 15s... Se persistir, reinicie o app.")
        time.sleep(15)
        st.rerun()
else:
    with placeholder_root.container():
