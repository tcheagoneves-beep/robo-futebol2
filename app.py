...
def gerar_bet_builder_alavancagem(api_key):
    ...
    try:
        ...
        for pick in top_picks:
            ...
            Use frases completas como "Vitória do Barcelona", "Mais de 2.5 Gols", "Mais de 3.5 Cartões".
            ...
            {{
                ...
                "selecoes": ["Vitória do [Time Específico]", "Mais de 2.5 Gols", "Mais de 3.5 Cartões"],
...
def verificar_alerta_matinal(token, chat_ids, api_key):
    ...
    if 6 <= agora.hour < 12:  # [FIX] Matinal estendido até 12h
        ...
        if not st.session_state['matinal_enviado']:
            ...
            if insights and "Sem jogos" not in str(insights) and "IA Offline" not in str(insights) and "dados suficientes" not in str(insights) and "Erro" not in str(insights)[:10]:
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
    ...
    if 12 <= agora.hour < 15:  # Janela estendida até 15h
        if not st.session_state.get('sniper_tarde_enviado', False):
            ...
            if insights_tarde and "Sem jogos" not in str(insights_tarde) and "IA Offline" not in str(insights_tarde) and "Erro" not in str(insights_tarde)[:10]:
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
    ...
    if 17 <= agora.hour < 18:
        if not st.session_state.get('sniper_noite_enviado', False):
            ...
            if insights_noite and "Sem jogos" not in str(insights_noite) and "IA Offline" not in str(insights_noite) and "Erro" not in str(insights_noite)[:10]:
                ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
