...
def bf_login():
    ...
    try:
        ...
        if not all([username, password, app_key]):
            st.session_state['bf_erro'] = f"Faltam: {', '.join(k for k,v in [('USER',username),('PASS',password),('KEY',app_key)] if not v)}"
            return False
        ...
        # ═══ TENTATIVA 1: Login via CERTIFICADO (.bet.br) ═══
        ...
        if cert_crt and cert_key_pem and "BEGIN" in str(cert_crt):
            try:
                ...
                print(f"[BETFAIR] 🔐 Cert .bet.br: {ls}")
                if ls == "SUCCESS":
        ...
        if client is None:
            ...
            for url in [
                ...
                try:
                    ...
                    dom = url.split('/')[2]
                    print(f"[BETFAIR] 📡 {dom}: {status} / {error}")
                    if status == "SUCCESS":
                        client = betfairlightweight.APIClient(
                            username=username, password=password, app_key=app_key,
                            lightweight=True
                        ...
                        erros.append(f"{dom}: {error}")
                except Exception as e_url:
                    erros.append(f"{url.split('/')[2]}: {str(e_url)[:50]}")
