...
def consultar_ia_gemini(dados_jogo, estrategia, stats_raw, rh, ra, extra_context="", time_favoravel=""):
    """
    CORREÇÃO LÓGICA: Verifica se a estratégia é UNDER (Morno) e ajusta a interpretação da IA.
    """
    ...
    # --- LOGICA CORRIGIDA UNDER VS OVER ---
    ...
    try:
        ...
        if 65 <= tempo <= 75:
            ...
            if (p_c and sh_h > sh_a) or (p_f and sh_a > sh_h):
                 if total_gols >= 1 or total_chutes >= 18: SINAIS.append({"tag": "💎 GOLDEN BET", "ordem": gerar_ordem_gol(total_gols, "Limite"), "stats": "🔥 Pressão Favorito + Finalizações", "rh": rh, "ra": ra, "favorito": "GOLS"}); golden_bet_ativada = True
        
        if not golden_bet_ativada and (70 <= tempo <= 75) and abs(gh - ga) <= 1:
            if total_chutes >= 22 and (not arame_liso_casa and not arame_liso_fora): SINAIS.append({"tag": "💰 Janela de Ouro", "ordem": gerar_ordem_gol(total_gols, "Limite"), "stats": f"🔥 {total_chutes} Chutes Totais", "rh": rh, "ra": ra, "favorito": "GOLS"})
        
        if 55 <= tempo <= 75:
             if total_chutes <= 10 and (sog_h + sog_a) <= 2 and gh == ga:
                 linha_under = total_gols + 0.5
                 SINAIS.append({"tag": "❄️ Jogo Morno", "ordem": f"👉 <b>FAZER:</b> Under Gols (Segurar)\n✅ Aposta: <b>Menos de {linha_under} Gols</b> (Ou Under Limite)", "stats": f"Jogo Travado ({total_chutes} chutes)", "rh": rh, "ra": ra, "favorito": "UNDER"})
        ...
        if tempo <= 30 and total_gols >= 2: SINAIS.append({"tag": "🟣 Porteira Aberta", "ordem": gerar_ordem_gol(total_gols), "stats": f"Jogo Aberto", "rh": rh, "ra": ra, "favorito": "GOLS"})
        if total_gols == 0 and (tempo <= 10 and total_chutes >= 3): SINAIS.append({"tag": "⚡ Gol Relâmpago", "ordem": gerar_ordem_gol(0, "HT"), "stats": "Início Intenso", "rh": rh, "ra": ra, "favorito": "GOLS"})
        
        if tempo <= 60:
            if gh <= ga and (rh >= 3 or (sh_h >= 8 and sog_h >= 3)) and not arame_liso_casa: SINAIS.append({"tag": "🟢 Blitz Casa", "ordem": gerar_ordem_gol(total_gols), "stats": "Pressão Casa", "rh": rh, "ra": ra, "favorito": "GOLS"})
            if ga <= gh and (ra >= 3 or (sh_a >= 8 and sog_a >= 3)) and not arame_liso_fora: SINAIS.append({"tag": "🟢 Blitz Visitante", "ordem": gerar_ordem_gol(total_gols), "stats": "Pressão Visitante", "rh": rh, "ra": ra, "favorito": "GOLS"})
        
        if 15 <= tempo <= 25 and total_chutes >= 6 and total_sog >= 3: SINAIS.append({"tag": "🏹 Tiroteio Elite", "ordem": gerar_ordem_gol(total_gols), "stats": "Muitos Chutes", "rh": rh, "ra": ra, "favorito": "GOLS"})
        ...
        if 10 <= tempo <= 40 and gh == ga:
            if (posse_h >= 55) and (sog_h >= 3) and (sh_h >= 6) and (sh_a <= 1) and rh >= 2: SINAIS.append({"tag": "🦁 Back Favorito (Nettuno)", "ordem": "👉 <b>FAZER:</b> Back Casa", "stats": "Dominância", "rh": rh, "ra": ra, "favorito": "CASA"})
            elif (posse_a >= 55) and (sog_a >= 3) and (sh_a >= 6) and (sh_h <= 1) and ra >= 2: SINAIS.append({"tag": "🦁 Back Favorito (Nettuno)", "ordem": "👉 <b>FAZER:</b> Back Visitante", "stats": "Dominância", "rh": rh, "ra": ra, "favorito": "VISITANTE"})

        if 60 <= tempo <= 88 and abs(gh - ga) >= 3 and (total_chutes >= 14): SINAIS.append({"tag": "🔫 Lay Goleada", "ordem": gerar_ordem_gol(total_gols, "Limite"), "stats": "Goleada Viva", "rh": rh, "ra": ra, "favorito": "GOLS"})
...
def processar_resultado(sinal, jogo_api, token, chats):
    ...
    if (gh + ga) > (ph + pa):
        ...
        # Lógica Correta para UNDER/MORNO (Gol é ruim)
        if "Morno" in strat or "Under" in strat:
            if (gh+ga) >= 2: # Exemplo: Over 1.5 bateu no Under
                sinal['Resultado'] = '❌ RED'
...
def verificar_automacao_bi(token, chat_ids, stake_padrao):
    ...
    if st.session_state['last_check_date'] != hoje_str:
        st.session_state.update({'bi_enviado': False, 'financeiro_enviado': False, 'bigdata_enviado': False, 'matinal_enviado': False, 'last_check_date': hoje_str})
...
def enviar_analise_estrategia(token, chat_ids):
    ...
    ids = [x.strip() for x in str(chat_ids).replace(';', ',').split(',') if x.strip()]
    for cid in ids: enviar_telegram(token, cid, f"🧪 <b>LABORATÓRIO IA</b>\n\n{sugestao}")
...
if st.session_state.ROBO_LIGADO:
    with placeholder_root.container():
        ...
        with abas[2]: # FINANCEIRO RESTAURADO
