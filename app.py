...
if '_bf_last_try' not in st.session_state: st.session_state['_bf_last_try'] = 0
...
def bf_login():
    """Login na Betfair via certificado SSL."""
    global BF_CONECTADO
    # Cooldown: impede tentativas repetidas (evita ban)
    import time as _time
    ultimo = st.session_state.get('_bf_last_try', 0)
    agora = _time.time()
    if agora - ultimo < 120:  # 2 minutos entre tentativas
        restante = int(120 - (agora - ultimo))
        st.session_state['bf_erro'] = f"Aguarde {restante}s antes de tentar novamente (proteção anti-ban)"
        return False
    st.session_state['_bf_last_try'] = agora
    ...
    try:
        ...
        if not all([username, password, app_key]):
            st.session_state['bf_erro'] = "Credenciais ausentes nos secrets"
            return False
        if not cert_crt or not cert_key_pem or "BEGIN" not in str(cert_crt):
            st.session_state['bf_erro'] = "Certificados CERT_CRT/CERT_KEY ausentes"
            return False
        st.session_state['bf_erro'] = ""
        print(f"[BETFAIR] 🔄 Login: {username[:5]}***")

        # Prepara certificados
        import tempfile
        cert_dir = tempfile.mkdtemp()
        crt_path = os.path.join(cert_dir, "client-2048.crt")
        key_path = os.path.join(cert_dir, "client-2048.key")
        with open(crt_path, 'w') as f: f.write(_fix_pem(str(cert_crt)))
        with open(key_path, 'w') as f: f.write(_fix_pem(str(cert_key_pem)))

        # Tenta cert login: .bet.br (Brasil) e .com (Global/Streamlit Cloud)
        erros = []
        client = None
        for cert_url in [
            ...
            try:
                ...
                try:
                    ...
                    erros.append(f"{dom}: HTTP {resp.status_code}")
                    continue
                ls = data.get("loginStatus", "NO_STATUS")
                print(f"[BETFAIR] 🔐 {dom}: {ls}")
                if ls == "SUCCESS":
                    ...
                    erros.append(f"{dom}: {ls}")
            except Exception as e:
                erros.append(f"{dom}: {str(e)[:60]}")

        if client is None:
            st.session_state['bf_erro'] = " | ".join(erros)
            return False
...
def _bf_atualizar_saldo():
    """Busca saldo atual da conta Betfair."""
    try:
...
def bf_keep_alive():
    ...
    try:
        ...
        st.session_state['bf_erro'] = "Sessão expirou. Clique em Conectar."
        return False
...
# Betfair: login SOMENTE via botão (auto-login removido para evitar ban)
...
with st.sidebar:
            ...
            if not st.session_state.get('bf_ativo'):
                ...
                if st.button("🔗 Conectar Betfair"):
                    with st.spinner("Conectando à Betfair..."):
                        ...
                        if ok:
                            ...
                            st.error(f"❌ {st.session_state.get('bf_erro', 'Erro')}")
                if st.session_state.get('bf_erro'):
                    st.caption(f"⚠️ {st.session_state['bf_erro']}")
            else:
                # ═══ CONECTADO ═══
                saldo = st.session_state.get('bf_saldo', 0)
                bets_hoje = len(st.session_state.get('bf_bets_hoje', []))
                total_dia = st.session_state.get('bf_total_apostado', 0)
                st.markdown(
                    f'<div style="background:#1a472a;border-radius:8px;padding:10px;margin:5px 0;text-align:center">'
                    f'<span style="color:#4ade80;font-size:22px;font-weight:bold">R$ {saldo:.2f}</span><br>'
                    f'<span style="color:#94a3b8;font-size:11px">SALDO BETFAIR</span>'
                    f'</div>', unsafe_allow_html=True
                )
                if bets_hoje > 0:
                    st.caption(f"📊 Hoje: {bets_hoje} apostas | R$ {total_dia:.2f} apostado")
                st.session_state['bf_auto_bet'] = st.checkbox(
                    "🤖 Apostar Automaticamente",
                    value=st.session_state.get('bf_auto_bet', False),
                    help="Quando ativado, o robô aposta na Betfair ao gerar um sinal aprovado"
                )
                if st.session_state['bf_auto_bet']:
                    st.session_state['bf_dry_run'] = st.checkbox("🧪 Modo Teste (não aposta de verdade)", value=st.session_state.get('bf_dry_run', True))
                    st.session_state['bf_so_aprovados'] = st.checkbox("✅ Só sinais aprovados pela IA", value=st.session_state.get('bf_so_aprovados', True))
                    st.session_state['bf_max_pct'] = st.slider("💰 % Máx da Banca:", 1, 10, st.session_state.get('bf_max_pct', 5))
                    st.session_state['bf_limit_dia'] = st.number_input("📊 Limite Diário (R$):", min_value=10.0, max_value=5000.0, value=st.session_state.get('bf_limit_dia', 100.0), step=10.0)
                ...
                with col_r2:
                    if st.button("🔌 Reconectar"):
                        st.session_state['bf_ativo'] = False
                        st.session_state['_bf_last_try'] = 0  # Reset cooldown
                        bf_login()
                        st.rerun()
    ...
    if st.session_state.get('bf_ativo'):
        bf_saldo = st.session_state.get('bf_saldo', 0)
        bf_mode = "🧪 TESTE" if st.session_state.get('bf_dry_run') else "💰 REAL"
        st.markdown(f'<div class="status-active">🅱️ BETFAIR CONECTADA<br>💰 R$ {bf_saldo:.2f} ({bf_mode})</div>', unsafe_allow_html=True)
    elif BF_AVAILABLE:
