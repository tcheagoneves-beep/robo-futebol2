...
def conectar_betfair_nuvem():
    import betfairlightweight
    import tempfile
    try:
        bf_user = st.secrets["BETFAIR"]["USERNAME"]
        bf_pass = st.secrets["BETFAIR"]["PASSWORD"]
        bf_key = st.secrets["BETFAIR"]["APP_KEY"]
        crt_content = st.secrets["BETFAIR"]["CERT_CRT"]
        key_content = st.secrets["BETFAIR"]["CERT_KEY"]

        with tempfile.NamedTemporaryFile(delete=False, suffix=".crt") as crt_file:
            crt_file.write(crt_content.encode())
        ...
        trading = betfairlightweight.APIClient(
            bf_user, bf_pass, app_key=bf_key, cert_files=[crt_path, key_path]
        )
        trading.login()
        return trading
...
# --- INICIALIZAR A CONEXÃO COM A BETFAIR ---
if 'betfair_token' not in st.session_state:
    st.session_state['betfair_token'] = conectar_betfair_nuvem()

if st.session_state['betfair_token']:
    st.success("🟢 Betfair Conectada e Pronta!")
else:
    st.warning("⚠️ Betfair não conectada. Verifique os Secrets no Streamlit Cloud.")
...
# --- FUNÇÃO DE LOGIN DA BETFAIR NA NUVEM ---
def conectar_betfair_nuvem():
    print("⏳ Iniciando login seguro na Betfair...")
    try:
        bf_secrets = st.secrets["BETFAIR"]
        app_key = bf_secrets["APP_KEY"]
        username = bf_secrets["USERNAME"]
        password = bf_secrets["PASSWORD"]
        crt_content = bf_secrets["CERT_CRT"]
        key_content = bf_secrets["CERT_KEY"]
    except KeyError:
        return None

    with tempfile.NamedTemporaryFile(delete=False, suffix=".crt") as crt_file:
        crt_file.write(crt_content.encode('utf-8'))
        crt_path = crt_file.name
    ...
    url_login = "https://identitysso-cert.betfair.bet.br/api/certlogin"
    headers_login = {
    ...
    session_token = None
    try:
        resp = requests.post(url_login, headers=headers_login, data=payload, cert=(crt_path, key_path), timeout=15)
...
# ==============================================================================
# [NOVO] MOTOR DE EXECUÇÃO BETFAIR (APOSTAS AUTOMÁTICAS)
# ==============================================================================
def realizar_aposta_betfair(nome_time_casa, estrategia, valor_stake, odd_limite):
    """Busca o jogo na Betfair e executa a aposta via JSON-RPC."""
    token = st.session_state.get('betfair_token')
    try:
        app_key = st.secrets["BETFAIR"]["APP_KEY"]
    except:
        return False, "App Key não encontrado nos secrets"

    if not token:
        return False, "Token inválido"

    url_rpc = "https://api.betfair.bet.br/exchange/betting/json-rpc/v1"
    headers = {
        "X-Application": app_key,
    ...
    def chamar_api(metodo, parametros):
        payload = {"jsonrpc": "2.0", "method": f"SportsAPING/v1.0/{metodo}", "params": parametros, "id": 1}
        try:
            res = requests.post(url_rpc, headers=headers, json=payload, timeout=10)
            return res.json().get("result")
        except: return None

    try:
        # 1. Achar o jogo usando o nome do time da casa
        eventos = chamar_api("listEvents", {"filter": {"textQuery": nome_time_casa}})
        if not eventos: return False, f"Jogo de {nome_time_casa} não encontrado na Betfair"
        event_id = eventos[0]['event']['id']

        # 2. Definir o mercado (Focando em Over 0.5 Gols para estratégias de Gols)
        # Adaptável para MATCH_ODDS dependendo do nome da estratégia
        tipo_mercado = "OVER_UNDER_05" if "Vovô" not in estrategia else "MATCH_ODDS"
        
        mercados = chamar_api("listMarketCatalogue", {
            "filter": {"eventIds": [event_id], "marketTypeCodes": [tipo_mercado]},
            "maxResults": 1,
            "marketProjection": ["RUNNER_METADATA"]
        })

        if not mercados: return False, "Mercado fechado ou indisponível"
        market_id = mercados[0]['marketId']

        # 3. Pegar o ID da seleção (Pegamos a opção 'Over' / 'Mais' ou o Time da Casa)
        selection_id = None
        for runner in mercados[0]['runners']:
            if "Over" in runner['runnerName'] or "Mais" in runner['runnerName'] or nome_time_casa in runner['runnerName']:
                selection_id = runner['selectionId']
                break
        if not selection_id: selection_id = mercados[0]['runners'][0]['selectionId']

        # 4. Disparar a Ordem Real
        stake_formatada = str(round(float(valor_stake), 2))
        odd_formatada = str(round(float(odd_limite), 2))
        ...
        ordem = {
            ...
            "instructions": [{
                ...
                "orderType": "LIMIT",
                "limitOrder": {
                    "size": stake_formatada,
                    "price": odd_formatada,
                    "persistenceType": "LAPSE" # Se a odd mudar, a aposta cancela para proteger o dinheiro
                }
            }]
        }

        resultado = chamar_api("placeOrders", ordem)

        if resultado and resultado.get('status') == 'SUCCESS':
            return True, f"Aposta de R$ {stake_formatada} correspondida @{odd_formatada}!"
        else:
            erro = resultado.get('errorCode') if resultado else "Erro desconhecido"
            return False, f"Falha na execução: {erro}"

    except Exception as e:
        return False, f"Erro interno na execução: {e}"
...
# --- 🚀 DISPARO AUTOMÁTICO NA BETFAIR ---
                                    if st.session_state.get('betfair_token') and st.session_state.ROBO_LIGADO:
                                        # Puxa o valor da Stake que a IA calculou (ou usa a padrão)
                                        valor_aposta = st.session_state.get('stake_padrao', 10.0)
                                        try: valor_aposta = float(stake_sinal) 
                                        except: pass
                                        
                                        st.toast(f"⏳ Executando ordem na Betfair: R$ {valor_aposta:.2f}...")
                                        sucesso, bf_msg = realizar_aposta_betfair(home, s['tag'], valor_aposta, odd_val)
                                        
                                        if sucesso:
                                            st.success(f"💰 BETFAIR: {bf_msg}")
                                            enviar_telegram(safe_token, safe_chat, f"🤖 <b>ROBÔ EXECUTOU NA BETFAIR:</b>\n{bf_msg}")
                                        else:
                                            st.warning(f"⚠️ BETFAIR (Ignorado): {bf_msg}")
