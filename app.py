...
def bf_login():
    """Login na Betfair via certificado SSL (único método que funciona na nuvem)."""
    global BF_CONECTADO
    if not BF_AVAILABLE:
    ...
    try:
        ...
        if not cert_crt or not cert_key_pem or "BEGIN" not in str(cert_crt):
            st.session_state['bf_erro'] = "Certificados CERT_CRT/CERT_KEY ausentes nos secrets"
            return False
        ...
        # Tenta cert login (BR primeiro, depois global)
        erros = []
        client = None
        for cert_url in [
        ...
        if BF_AVAILABLE:
            ...
            if not st.session_state.get('bf_ativo'):
                ...
                st.session_state['bf_auto_bet'] = st.checkbox(
                    ...
                    help="Quando ativado, o robô aposta na Betfair automaticamente quando gera um sinal"
                )
