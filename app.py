...
def verificar_mercados_bet365(api_key, fid):
    ...
    Usa 1 API call por jogo.
    """
    resultado = {'cartoes': False, 'escanteios': False, 'jogador': False, 'gols': False}
    try:
        url = "https://v3.football.api-sports.io/odds"
        params = {"fixture": fid, "bookmaker": "8"}  # Bet365
        r = requests.get(url, headers={"x-apisports-key": api_key}, params=params).json()
        
        if not r.get('response'): return resultado
        bookmakers = r['response'][0].get('bookmakers', [])
        if not bookmakers: return resultado
        
        bets = bookmakers[0].get('bets', [])
        bet_ids = {b['id'] for b in bets}
        bet_names = {b['name'].lower() for b in bets}
        
        # Mapear mercados disponíveis
        # IDs comuns: 1=Match Winner, 5=Goals O/U, 28=Total Cards, 29=Corners O/U
        # Player props variam, mas geralmente contêm "player" ou "shots" no nome
        
        resultado['gols'] = 5 in bet_ids or 1 in bet_ids
        
        # Cartões - IDs 28, 115 (Total Booking Points), ou nome contém "card"
        resultado['cartoes'] = (28 in bet_ids or 115 in bet_ids or 
            any('card' in n for n in bet_names) or 
            any('booking' in n for n in bet_names))
        
        # Escanteios - ID 29 ou nome contém "corner"
        resultado['escanteios'] = (29 in bet_ids or 
            any('corner' in n for n in bet_names))
        
        # Jogador (chutes, defesas) - nome contém "player", "shot", "save", "anytime"
        resultado['jogador'] = (
            any('player' in n for n in bet_names) or
            any('shot' in n for n in bet_names) or
            any('anytime' in n for n in bet_names) or
            any('goalscorer' in n for n in bet_names))
        
    except Exception as e:
        print(f"[NEVES] Erro verificar_mercados_bet365 {fid}: {e}")
...
def gerar_analise_mercados_alternativos_ia(api_key):
    ...
    try:
        ...
        SUA MISSÃO — ENCONTRAR 4 A 6 OPORTUNIDADES nos seguintes mercados:
        ...
        2. 🧤 DEFESAS DO GOLEIRO — SÓ se "✅ JOGADOR":
           - Cenário "Massacre Casa" → Goleiro Visitante Over 3.5 Defesas
           - Cenário "Massacre Visitante" → Goleiro Casa Over 3.5 Defesas
           - Indique: "Goleiro do [Time] Over 3.5 Defesas" (NÃO nome do jogador)
        ...
        REGRAS:
        - Mínimo 4, máximo 6 sinais.
        - NUNCA sugira um mercado com ❌ na Bet365 para aquele jogo.
        - Inclua pelo menos 1 de FINALIZAÇÕES (se houver jogo com ✅ JOGADOR).
        - Inclua pelo menos 1 de CARTÕES (se houver jogo com ✅ CARTÕES).
        - Cada sinal deve ter um jogo DIFERENTE.
...
def markdown_to_telegram_html(texto):
    ...
    # 1. PRIMEIRO: Converter bullets markdown (* item) → emoji bullet (ANTES do bold/italic)
...
def enviar_alerta_alternativos(token, chat_ids, api_key):
    ...
    if not sinais: 
        ...
        st.session_state['alternativos_enviado'] = True  # Marca para não travar em loop
        return
    ...
    print(f"[NEVES ALT] Enviados {enviados}/{len(sinais)} sinais alternativos.")
