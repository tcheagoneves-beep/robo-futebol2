...
def bf_login():
    """Login na Betfair via API — tenta cert (.bet.br + .com) depois REST."""
    global BF_CONECTADO
    erros = []
    if not BF_AVAILABLE:
    ...
    try:
        ...
        print(f"[BETFAIR] 🔄 Login: {username[:5]}*** | Key: {app_key[:6]}***")

        client = None

        # ═══ CERT LOGIN (tenta .bet.br depois .com) ═══
        ...
        if cert_crt and cert_key_pem and "BEGIN" in str(cert_crt):
            ...
            for cert_url in cert_endpoints:
                try:
                    ...
                    # Trata resposta não-JSON (HTML de bloqueio)
                    try:
                        data = resp.json()
                    except:
                        erros.append(f"Cert {dom}: resposta não-JSON (HTTP {resp.status_code})")
                        print(f"[BETFAIR] ⚠️ Cert {dom}: HTTP {resp.status_code}, body: {resp.text[:100]}")
                        continue
                    ls = data.get("loginStatus", "NO_STATUS")
                    print(f"[BETFAIR] 🔐 Cert {dom}: {ls}")
                    if ls == "SUCCESS":
                        ...
                        print(f"[BETFAIR] ✅ Cert OK via {dom}!")
                        break
                    else:
                        erros.append(f"Cert {dom}: {ls}")
                except Exception as e_cert:
                    erros.append(f"Cert {dom}: {str(e_cert)[:60]}")
                    print(f"[BETFAIR] ⚠️ Cert {dom}: {e_cert}")

        # ═══ REST LOGIN (fallback sem cert) ═══
        if client is None:
            print("[BETFAIR] 🔄 REST login...")
            rest_endpoints = [
                "https://identitysso.betfair.bet.br/api/login",
            ...
            for url in rest_endpoints:
                try:
                    ...
                    try:
                        ...
                        erros.append(f"REST {dom}: não-JSON (HTTP {resp.status_code})")
                        print(f"[BETFAIR] ⚠️ REST {dom}: HTTP {resp.status_code}")
                        continue
                    status = data.get("status", data.get("loginStatus", ""))
                    error = data.get("error", data.get("loginStatus", ""))
                    print(f"[BETFAIR] 📡 REST {dom}: {status} / {error}")
                    if status == "SUCCESS":
                        client = betfairlightweight.APIClient(
                            username=username, password=password, app_key=app_key,
                            lightweight=True
                        ...
                        print(f"[BETFAIR] ✅ REST OK via {dom}!")
                        break
                    else:
                        erros.append(f"REST {dom}: {error}")
                except Exception as e_url:
                    erros.append(f"REST {url.split('/')[2]}: {str(e_url)[:50]}")

        if client is None:
            st.session_state['bf_erro'] = " | ".join(erros)
            print(f"[BETFAIR] ❌ FALHOU: {st.session_state['bf_erro']}")
            return False
                ...
                if st.button("🔗 Conectar Betfair"):
                    with st.spinner("Conectando à Betfair..."):
                        ...
                        if ok:
                            ...
                            st.error(f"❌ {st.session_state.get('bf_erro', 'Erro desconhecido')}")
