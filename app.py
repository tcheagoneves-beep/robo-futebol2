...
def calcular_stake_recomendado(banca_atual, probabilidade, odd, modo='fracionario', opiniao_ia='Aprovado'):
    ...
    try:
        ...
        if opiniao_ia == 'Aprovado':
            # IA confia: Kelly determina (mín 1%, máx 5%)
...
def gerar_insights_matinais_ia(api_key):
    ...
    try:
        ...
        for j in jogos_candidatos:
            if count >= 25: break  # [OTIMIZADO] Max 25 jogos
            ...
            # 1. Referência de Preço (tenta, mas NÃO bloqueia)
            odd_val = 0
            try: odd_val, odd_nome = buscar_odd_pre_match(api_key, fid)
            except: pass
            
            # 2. DADOS MACRO (50 JOGOS - cached 24h)
            macro = analisar_tendencia_50_jogos(api_key, home_id, away_id)
            if not macro: continue  # Sem dados históricos = skip
            
            # 3. DADOS MICRO (10 JOGOS - cached)
            micro = None
            ...
            if macro:
                ...
                # Odd (tenta mas não bloqueia)
                odd_txt = "N/D"
                if odd_val > 0: odd_txt = f"@{odd_val:.2f}"
                
                # Micro (tenta mas tolera falha)
                micro_txt = ""
                try:
                    if micro:
            ...
            💰 Ref: {odd_txt}
            📅 LONGO PRAZO (50 Jogos):
            - Casa: Over 1.5 FT: {h_50.get('over15_ft', 0)}% | Over 0.5 HT: {h_50.get('over05_ht', 0)}% | BTTS: {h_50.get('ambas_marcam', 0)}% | ({h_50.get('qtd', 0)} jogos)
            - Fora: Over 1.5 FT: {a_50.get('over15_ft', 0)}% | Over 0.5 HT: {a_50.get('over05_ht', 0)}% | BTTS: {a_50.get('ambas_marcam', 0)}% | ({a_50.get('qtd', 0)} jogos)
            {micro_txt}
            """
                count += 1
        ...
        2. ⚡ **GOLS (OVER):**
           - Busque times com alta taxa de Over 1.5 FT no longo prazo (>70%) E alto Over 0.5 HT (>60%).
           
        3. ❄️ **UNDER (TRINCHEIRA):**
           - Busque jogos onde Over 1.5 FT é baixo (<50%) e Over 0.5 HT é baixo (<40%).
        ...
        # [VALIDAÇÃO V2] Removida - gerava warning falso e gastava API call extra
        # Veredicto agora é data-driven (calcular_veredicto_datadriven)

        # ═══════════════════════════════════════════════════════════════════════
        # [IA COMPACTA] Análise resumida por jogo (skip se falhar)
        # ═══════════════════════════════════════════════════════════════════════
        
        import re
        # [OTIMIZADO] IA por jogo desativada (6 zonas do Gemini já são completas)
        # Isso economiza ~50 API calls e 3-5 min de processamento
        print(f"[SNIPER] ✅ Texto Gemini pronto ({len(texto_ia)} chars)")
...
def calcular_veredicto_datadriven(estrategia, liga, odd_val=0):
    """
    Veredicto baseado em DADOS REAIS do histórico.
    Retorna: (veredicto: str, probabilidade: int, fontes: str)
    
    Lógica:
    - Pega winrate da estratégia no histórico
    - Pega winrate da estratégia NESTA liga
    - Pega winrate da faixa de odd
    - Faz média ponderada → veredicto
    """
    df = st.session_state.get('historico_full', pd.DataFrame())
    if df.empty or len(df) < 10:
        return "Aprovado", 65, "Sem dados"  # Sem histórico = aprovado por padrão
    
    df_final = df[df['Resultado'].isin(['✅ GREEN', '❌ RED'])].copy()
    if len(df_final) < 10:
        return "Aprovado", 65, "Poucos dados"
    
    scores = []
    pesos = []
    fontes = []
    
    # ═══ 1. WINRATE DA ESTRATÉGIA (peso 3) ═══
    ...
    # ═══ 2. WINRATE ESTRATÉGIA + LIGA (peso 5 - mais importante) ═══
    df_strat_liga = df_final[(df_final['Estrategia'] == estrategia) & (df_final['Liga'] == liga)]
    if len(df_strat_liga) >= 3:
        g_sl = len(df_strat_liga[df_strat_liga['Resultado'].str.contains('GREEN')])
        wr_sl = (g_sl / len(df_strat_liga)) * 100
        scores.append(wr_sl)
        pesos.append(5)
        fontes.append(f"Liga:{wr_sl:.0f}%({len(df_strat_liga)}j)")
    
    # ═══ 3. WINRATE POR FAIXA DE ODD (peso 2) ═══
    if odd_val > 1.0:
        try:
            ...
            # Define faixa
            if odd_val < 1.30: faixa_min, faixa_max = 1.01, 1.35
            elif odd_val < 1.50: faixa_min, faixa_max = 1.30, 1.55
            elif odd_val < 1.80: faixa_min, faixa_max = 1.50, 1.85
            else: faixa_min, faixa_max = 1.80, 5.0
            
            df_faixa = df_final[df_final['Odd_Num'].between(faixa_min, faixa_max)]
            if len(df_faixa) >= 5:
                g_f = len(df_faixa[df_faixa['Resultado'].str.contains('GREEN')])
                wr_f = (g_f / len(df_faixa)) * 100
                scores.append(wr_f)
                pesos.append(2)
                fontes.append(f"Odd:{wr_f:.0f}%")
        except: pass
    
    # ═══ 4. WINRATE GERAL do robô (peso 1 - baseline) ═══
    g_total = len(df_final[df_final['Resultado'].str.contains('GREEN')])
    wr_total = (g_total / len(df_final)) * 100
    scores.append(wr_total)
    pesos.append(1)
    
    # ═══ CÁLCULO FINAL (média ponderada) ═══
    if not scores:
        return "Aprovado", 65, "Sem dados"
    
    prob_final = sum(s * p for s, p in zip(scores, pesos)) / sum(pesos)
    
    # ═══ VEREDICTO ═══
    # Regras com ligas bloqueadas pelo aprendizado
    ...
    return veredicto, int(prob_final), "+".join(fontes)
...
def consultar_ia_gemini(dados_jogo, estrategia, stats_raw, rh, ra, extra_context="", time_favoravel="", liga_nome=""):
    ...
    try:
        ...
        response = model_ia.generate_content(prompt, generation_config=genai.types.GenerationConfig(temperature=0.4)) # Aumentei a temperatura para ela ser mais criativa
        ...
        try:
            ...
            veredicto_data, prob_data, fontes_data = calcular_veredicto_datadriven(estrategia, liga_nome, odd_ctx)
...
def analisar_financeiro_com_ia(stake_padrao, banca_inicial):
    ...
    try:
        ...
        for _, row in df_aprov.iterrows():
            ...
            # Odd REAL
            try: odd_real = float(str(row.get('Odd', '1.50')).replace(',', '.'))
            except: odd_real = 1.50
            
            # Stake REAL (Kelly)
            try:
                prob = float(str(row.get('Probabilidade', '60')).replace('%', '').replace('...', '0'))
...
def verificar_alerta_matinal(token, chat_ids, api_key):
    ...
    if 6 <= agora.hour < 12:  # [FIX] Matinal estendido até 12h

        if not st.session_state['matinal_enviado']:
            try:
                print(f"[SNIPER MATINAL] Iniciando... {agora.strftime('%H:%M')}")
                result_matinal = gerar_insights_matinais_ia(api_key)
                insights = result_matinal[0] if isinstance(result_matinal, tuple) else result_matinal
                print(f"[SNIPER MATINAL] Resultado: {len(str(insights))} chars | Preview: {str(insights)[:100]}...")
                bloqueios = ["Sem jogos", "Nenhum jogo", "IA Offline", "dados suficientes"]
                tem_conteudo = insights and not any(b in str(insights) for b in bloqueios)
                if tem_conteudo:
                    # [FIX] Envia por ZONAS separadas (evita HTML quebrado)
                    try:
                        n_partes = enviar_sniper_por_zonas(token, chat_ids, str(insights))
                        print(f"[SNIPER MATINAL] ✅ Enviado em {n_partes} partes!")
                    except Exception as ez:
                        print(f"[SNIPER MATINAL] ⚠️ Zona fallback: {ez}")
                        # Fallback: envia direto (enviar_telegram já divide em 4000 chars)
                        msg_final = f"🌅 <b>SNIPER MATINAL (IA + DADOS)</b>\n\n{insights}"
                        enviar_telegram(token, chat_ids, msg_final)
                        print(f"[SNIPER MATINAL] ✅ Enviado via fallback!")
                    try: salvar_snipers_do_texto(str(insights))
                    except: pass
                else:
                    print(f"[SNIPER MATINAL] ⚠️ Sem conteúdo: {str(insights)[:200]}")
    ...
    if 12 <= agora.hour < 15:  # Janela estendida até 15h
        if not st.session_state.get('sniper_tarde_enviado', False):
            try:
                print(f"[SNIPER TARDE] Iniciando... {agora.strftime('%H:%M')}")
                result_tarde = gerar_insights_matinais_ia(api_key)
                insights_tarde = result_tarde[0] if isinstance(result_tarde, tuple) else result_tarde
                print(f"[SNIPER TARDE] Resultado: {str(insights_tarde)[:100]}...")
                
                # Filtros de conteúdo vazio
                bloqueios = ["Sem jogos", "Nenhum jogo", "IA Offline", "dados suficientes"]
                tem_conteudo = insights_tarde and not any(b in str(insights_tarde) for b in bloqueios)
                
                if tem_conteudo:
                    try:
                        n_p = enviar_sniper_por_zonas(token, chat_ids, str(insights_tarde), "☀️ <b>SNIPER DA TARDE (IA + DADOS)</b>")
                        print(f"[SNIPER TARDE] ✅ Enviado em {n_p} partes!")
                    except Exception as ez:
                        print(f"[SNIPER TARDE] ⚠️ Fallback: {ez}")
                        enviar_telegram(token, chat_ids, f"☀️ <b>SNIPER DA TARDE</b>\n\n{insights_tarde}")
                    try: salvar_snipers_do_texto(insights_tarde)
                    except: pass
                    st.toast("☀️ Sniper da Tarde enviado!")
                    print(f"[SNIPER TARDE] ✅ Enviado em {n_p} partes!")
                else:
                    print(f"[SNIPER TARDE] ⚠️ Sem conteúdo válido: {str(insights_tarde)[:200]}")
                    st.toast("☀️ Sniper Tarde: sem jogos novos encontrados")
                
                # Marca como enviado SEMPRE (evita retry infinito)
                st.session_state['sniper_tarde_enviado'] = True
                
            except Exception as e:
                print(f"[SNIPER TARDE] ❌ ERRO: {e}")
                import traceback; traceback.print_exc()
                st.toast(f"⚠️ Erro Sniper Tarde: {str(e)[:50]}")
                st.session_state['sniper_tarde_enviado'] = True  # Não tenta de novo

    # 3. SNIPER DA NOITE (17h-18h) — Jogos noturnos
    if 17 <= agora.hour < 18:
        if not st.session_state.get('sniper_noite_enviado', False):
            try:
                print(f"[SNIPER NOITE] Iniciando... {agora.strftime('%H:%M')}")
                result_noite = gerar_insights_matinais_ia(api_key)
                insights_noite = result_noite[0] if isinstance(result_noite, tuple) else result_noite
                print(f"[SNIPER NOITE] Resultado: {str(insights_noite)[:100]}...")
                bloqueios = ["Sem jogos", "Nenhum jogo", "IA Offline", "dados suficientes"]
                tem_conteudo = insights_noite and not any(b in str(insights_noite) for b in bloqueios)
                if tem_conteudo:
                    try:
                        n_p = enviar_sniper_por_zonas(token, chat_ids, str(insights_noite), "🌙 <b>SNIPER DA NOITE (IA + DADOS)</b>")
                        print(f"[SNIPER NOITE] ✅ Enviado em {n_p} partes!")
                    except Exception as ez:
                        print(f"[SNIPER NOITE] ⚠️ Fallback: {ez}")
                        enviar_telegram(token, chat_ids, f"🌙 <b>SNIPER DA NOITE</b>\n\n{insights_noite}")
                    try: salvar_snipers_do_texto(insights_noite)
                    except: pass
                    st.toast("🌙 Sniper da Noite enviado!")
                    print(f"[SNIPER NOITE] ✅ Enviado em {n_p} partes!")
                else:
                    print(f"[SNIPER NOITE] ⚠️ Sem conteúdo: {str(insights_noite)[:200]}")
                st.session_state['sniper_noite_enviado'] = True
            except Exception as e:
                print(f"[SNIPER NOITE] ❌ ERRO: {e}")
                import traceback; traceback.print_exc()
                st.session_state['sniper_noite_enviado'] = True
            ...
            if res_final:
                ...
                try:
                    ...
                    if key_apura not in st.session_state['banca_updates']:
                        ...
                        # [FIX] Banca usa Kelly inteligente (1-5% Aprovado, 1% Arriscado)
                        stake_val = max(float(st.session_state.get('banca_atual', 100.0)) * 0.01, 1.0)  # Default 1%
                        try:
                            prob_banca = float(str(s.get('Probabilidade', '50')).replace('%','').replace('...','0'))
                            ...
                            kelly_b = calcular_stake_recomendado(banca_para_calc, prob_banca, odd_local, kelly_modo_b, opiniao_banca)
                ...
                if deve_enviar:
                    ...
                    if tipo_msg == "GREEN" and key_green not in st.session_state['alertas_enviados']:
                         try:
                            ...
                            odd_g = 1.50
                            try: odd_g = float(str(s.get('Odd','1.50')).replace(',','.'))
                            except: pass
                         ...
                         try:
                            ...
                            odd_r_local = 1.50
                            try: odd_r_local = float(str(s.get('Odd','1.50')).replace(',','.'))
                            except: pass
            ...
            for j in jogos_live:
                ...
                if lista_sinais:
                    ...
                    for s in lista_sinais:
                        ...
                        kelly_stake_salvar = ""
                        try:
                            prob_k = float(str(prob_txt).replace('%','').replace('...','0'))
