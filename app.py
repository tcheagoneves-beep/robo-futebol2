...
def bf_login():
    """Login na Betfair via API (interactive login — sem certificados)."""
    global BF_CONECTADO
    if not BF_AVAILABLE:
        print("[BETFAIR] ❌ betfairlightweight não instalado")
        return False
    try:
        username = st.secrets.get("BF_USERNAME", "")
        password = st.secrets.get("BF_PASSWORD", "")
        app_key = st.secrets.get("BF_APP_KEY", "")
        if not all([username, password, app_key]):
            print("[BETFAIR] ❌ Secrets BF_USERNAME/BF_PASSWORD/BF_APP_KEY ausentes")
            return False

        # Tenta login via cert se disponível
        cert_pem = st.secrets.get("BF_CERT_PEM", "")
        cert_key = st.secrets.get("BF_CERT_KEY", "")
        if cert_pem and cert_key:
            ...
            # Fallback: interactive login via REST
            resp = requests.post(
            ...
            if data.get("status") != "SUCCESS":
                # Tenta endpoint .com.br (Betfair Brasil)
                resp = requests.post(
                    "https://identitysso.betfair.com.br/api/login",
                    headers={"X-Application": app_key, "Accept": "application/json"},
                    data={"username": username, "password": password}
                )
                data = resp.json()
            if data.get("status") != "SUCCESS":
                print(f"[BETFAIR] ❌ Login falhou: {data.get('error', 'Erro desconhecido')}")
                return False
        ...
        BF_CONECTADO = True
        print(f"[BETFAIR] ✅ Login OK (token: {str(st.session_state['bf_session_token'])[:12]}...)")
        return True
    except Exception as e:
...
def bf_apostar(market_id, selection_id, stake, odd_limite=1.01):
    ...
    if st.session_state.get('bf_dry_run', True):
        ...
        return resultado
    if stake > st.session_state.get('bf_max_stake', 10.0):
        stake = st.session_state['bf_max_stake']
...
def bf_executar_aposta_sinal(home, away, estrategia, ordem, odd_val, stake_val, opiniao_db, token, chat_ids):
    """Pipeline completo: busca evento → mercado → aposta. Retorna True se apostou."""
    ...
    if st.session_state.get('bf_so_aprovados') and opiniao_db != "Aprovado":
        print(f"[BETFAIR] ⏭️ Sinal não aprovado pela IA ({opiniao_db}), pulando.")
        return False
    try:
        ...
        if not event_id:
            print(f"[BETFAIR] ❌ Evento não encontrado: {home} vs {away}")
            return False
        # 2. Determina mercado
        tipo = classificar_tipo_estrategia(estrategia)
        ...
        if not market_id:
            # Fallback: tenta Over/Under 1.5
            market_id, runners = bf_buscar_mercado(event_id, "OVER_UNDER_15")
        if not market_id:
            print(f"[BETFAIR] ❌ Mercado {market_type} não encontrado")
            return False
        # 3. Seleciona runner
        ...
        if not selection_id:
            ...
            return False
        # 4. Obtém odd real da Betfair
        bf_odd = bf_obter_melhor_odd(market_id, selection_id)
        if bf_odd < 1.01:
            print(f"[BETFAIR] ❌ Odd indisponível (mercado fechado?)")
            return False
        # 5. Stake (usa Kelly ou max_stake, o menor)
        stake_final = min(stake_val, st.session_state.get('bf_max_stake', 10.0))
        if stake_final < 2.0: stake_final = 2.0  # Mínimo Betfair
        # 6. Aposta!
        resultado = bf_apostar(market_id, selection_id, stake_final, bf_odd)
        # 7. Notifica via Telegram
        dry_tag = "🧪 DRY RUN" if st.session_state.get('bf_dry_run') else "💰 REAL"
        if resultado:
            ...
            msg_bf = (
                ...
                f"💰 Stake: R$ {stake_final:.2f} @ {bf_odd:.2f}\n"
                f"📊 Status: {status_txt}\n"
                f"🔢 Total Dia: R$ {st.session_state.get('bf_total_apostado', 0):.2f}"
            )
...
if BF_AVAILABLE and "BF_USERNAME" in st.secrets:
    try:
...
with st.sidebar:
    ...
    with st.expander("⚙️ Configurações", expanded=True):
        ...
        if BF_AVAILABLE and "BF_USERNAME" in st.secrets:
            ...
            if st.session_state['bf_auto_bet']:
                ...
                st.session_state['bf_max_stake'] = st.number_input("💰 Stake Máx (R$):", min_value=2.0, max_value=500.0, value=st.session_state.get('bf_max_stake', 10.0), step=5.0)
    ...
    if st.session_state.get('bf_ativo'):
        ...
        st.markdown(f'<div class="status-active">🅱️ BETFAIR CONECTADA ({bf_mode})</div>', unsafe_allow_html=True)
    elif BF_AVAILABLE and "BF_USERNAME" in st.secrets:
                            ...
                            try:
                                ...
                                try:
                                    if st.session_state.get('bf_ativo') and st.session_state.get('bf_auto_bet'):
                                        bf_stake = stake_sinal if stake_sinal > 0 else st.session_state.get('bf_max_stake', 10.0)
                                        bf_executar_aposta_sinal(
                                            home, away, s['tag'], s.get('ordem', ''),
                                            odd_val, bf_stake, opiniao_db,
